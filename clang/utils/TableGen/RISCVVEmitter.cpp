...
void RVVEmitter::createRVVIntrinsics(
  ...
  for (auto *R : RV) {
    ...
    SmallVector<PrototypeDescriptor> MaskedPrototype =
        RVVIntrinsic::computeBuiltinTypes(BasicPrototype, /*IsMasked=*/true,
    ...
    for (char I : TypeRange) {
      for (int Log2LMUL : Log2LMULList) {
        ...
        Out.push_back(std::make_unique<RVVIntrinsic>(
            ...
            ManualCodegen, *Types, IntrinsicTypes, RequiredFeatures,
            NF, IsTU));
        if (HasMasked) {
...
void RVVEmitter::createRVVIntrinsicRecords(std::vector<RVVIntrinsicRecord> &Out,
  ...
  for (const auto &SR : SemaRecords) {
    ...
    assert(R.OverloadedSuffixIndex != SemaSignatureTable::INVALID_INDEX);
  }
}
