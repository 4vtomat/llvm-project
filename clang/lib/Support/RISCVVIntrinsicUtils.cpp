...
PrototypeDescriptor::parsePrototypeDescriptor(
  ...
  // (SIFIVE) Compute the vector type transformers
  // (SIFIVE) For SiFive custom instruction contains multiple VectorTypeModifier
  ...
  if (PrototypeDescriptorStr.startswith("(")) {
    assert(VTM == VectorTypeModifier::NoModifier || // SIFIVE
           PType == 'q' || PType == 'w'); // SIFIVE
    ...
    if (ComplexTT.first == "Log2EEW") {
      ...
      } else if (Log2LMUL == 1 && PType == 'q') {
        VTM = VectorTypeModifier::Widening4XVectorMultipleLMUL1;
      } else if (Log2LMUL == 2 && PType == 'q') {
        VTM = VectorTypeModifier::Widening4XVectorMultipleLMUL2;
      } else {
...
void RVVType::applyModifier(const PrototypeDescriptor &Transformer) {
  ...
  case VectorTypeModifier::Widening2XVector:
    ...
    // ex: vbfloat16m1_t sf_vfncvt_bf16_f_w_bf16m1 (vfloat32m2_t src, size_t
    // vl); Parameter needs to be converted to 32-bit and bfloat don't have
    // 32-bit.
  ...
  case VectorTypeModifier::Widening4XVectorMultipleLMUL1:
    ...
    Scale = LMUL.getScale(ElementBitwidth);
    break;
  case VectorTypeModifier::Widening4XVectorMultipleLMUL2:
    ElementBitwidth *= 4;
    LMUL.MulLog2LMUL(1);
    Scale = LMUL.getScale(ElementBitwidth);
    break;
...
RVVIntrinsic::RVVIntrinsic(
  ...
  updateNamesAndPolicy(IsMasked, hasPolicy(), IsPrototypeDefaultTU, Name,
                       BuiltinName, OverloadedName, PolicyAttrs);
...
llvm::SmallVector<PrototypeDescriptor> RVVIntrinsic::computeBuiltinTypes(
    ...
    bool IsPrototypeDefaultTU, PolicyScheme DefaultScheme, Policy PolicyAttrs) {
  ...
  if (PolicyAttrs.isPolicyNonePolicy()) {
    if (!IsMasked) {
      ...
      if (IsPrototypeDefaultTU)
        PolicyAttrs.TailPolicy = Policy::PolicyType::Undisturbed; // TU
      else
  ...
  if (IsMasked) {
    ...
    if (NF == 1 && IsPrototypeDefaultTU && PolicyAttrs.isTAMAPolicy() &&
    ...
    if (NF == 1) {
      if (PolicyAttrs.isTUPolicy() && HasPassthruOp && !IsPrototypeDefaultTU)
