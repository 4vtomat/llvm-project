...
bool RISCVTTIImpl::getMemoryRefInfo(
  ...
  case Intrinsic::riscv_vsseg8: {
    ...
    if (SegNum > 1)
      Stride = ConstantInt::get(XLenIntTy, ElemSize * SegNum);
    for (unsigned n = 0; n < SegNum; ++n) {
      Value *Index = nullptr;
      if (n)
        Index = ConstantInt::get(XLenIntTy, ElemSize * n);
  ...
  case Intrinsic::riscv_vsuxseg8: {
    ...
    if (HasMask)
      Mask = II->getArgOperand(VLIndex - 1);
    Value *EVL = II->getArgOperand(VLIndex);
    // The vl operand of segment load/store means for each segment.
    unsigned SegNum = getSegNum(II, PtrOperandNo, IsWrite);
    unsigned ElemSize = Ty->getScalarSizeInBits() / 8;
    for (unsigned n = 0; n < SegNum; ++n) {
...
InstructionCost RISCVTTIImpl::getCastInstrCost(unsigned Opcode, Type *Dst,
  ...
  case ISD::FP_TO_UINT: {
      ...
      if (DstEltSize == 1) {
        return SrcLT.first * 3 * SrcLMULCost;
      }
      int PowDiff =
          std::abs((int)Log2_32(DstEltSize) - (int)Log2_32(SrcEltSize));
      if (PowDiff <= 1)
        return SrcLT.first * SrcLMULCost;
      // Backend could lower (v[sz]ext i8 to double) to vfcvt(v[sz]ext.f8 i8),
      // so it only need two conversion.
      if (SrcLT.second.isInteger() && (SrcEltSize < DstEltSize)) {
        if (CostKind != TTI::TCK_RecipThroughput) {
          return 2;
        } else {
          InstructionCost ExtendCost = SrcLMULCost;
...
#ifdef SIFIVE_CUSTOMIZATION
bool RISCVTTIImpl::canMacroFuseCmp() const {
  return true;
}
#endif // SIFIVE_CUSTOMIZATION
