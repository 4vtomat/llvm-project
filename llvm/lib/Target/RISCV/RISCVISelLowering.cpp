...
#if SIFIVE_CUSTOMIZATION
    setOperationAction({ISD::STRICT_FRINT, ISD::STRICT_FNEARBYINT,
                        ISD::STRICT_FCEIL, ISD::STRICT_FFLOOR,
                        ISD::STRICT_FROUND, ISD::STRICT_FROUNDEVEN,
                        ISD::STRICT_FTRUNC},
                       MVT::f16, Promote);
#endif
...
SDValue RISCVTargetLowering::lowerAArch64_qrshl(SelectionDAG &DAG,
  ...
  auto toVectorIfScalar = [&](SDValue V) {
    if (V.getSimpleValueType().isVector())
      return V;
    return DAG.getSplatVector(Op0.getSimpleValueType(), DL, V);
  };
  MVT XLenVT = Subtarget.getXLenVT();
  MVT VecVT = Op0.getSimpleValueType();
  MVT VT = Op1.getSimpleValueType();
  SDValue Zero = DAG.getConstant(0, DL, VecVT);
  unsigned Size = VecVT.getScalarSizeInBits();
  ...
  case Intrinsic::aarch64_neon_urshl:
    ...
    LShift = DAG.getSelectCC(
        DL, Op1, MaxShift, DAG.getConstant(0, DL, VecVT),
        DAG.getNode(ISD::SHL, DL, VecVT, Op0, toVectorIfScalar(Op1)),
        ISD::SETUGT);
    break;
  }
  SDValue RShift;
  ...
  case Intrinsic::aarch64_neon_urshl: {
    ...
    SDValue SShift = convertFromScalableVector(
        VecVT,
        DAG.getNode(
            IsSigned ? RISCVISD::VSSRA_VL : RISCVISD::VSSRL_VL, DL,
            ContainerVecVT,
            {convertToScalableVector(ContainerVecVT, Op0, DAG, Subtarget),
    ...
    if (!IsSigned) {
      ...
      // use Op0 >> MaxShift to calculate result.
      SShift = DAG.getSelectCC(
          DL, RShiftAmount, DAG.getConstant(Size, DL, VT),
          DAG.getNode(ISD::SRL, DL, VecVT, Op0, toVectorIfScalar(MaxShift)),
          SShift, ISD::SETEQ);
    }
...
static SDValue performVP_TRUNCATECombine(SDNode *N, SelectionDAG &DAG,
  ...
  if (In.getOpcode() != ISD::VP_LSHR || In.getOperand(2) != Mask ||
      In.getOperand(3) != VL)
    return SDValue();
  ...
  auto I = llvm::find_if(Operands, [](const SDValue &Op) {
                           return isOneOrOneSplat(Op);
                         });
  ...
  if (Op0.getValueType() != VT)
    Op0 = DAG.getNode(ISD::VP_ZERO_EXTEND, SDLoc(Operands[0]), VT, Op0, Mask,
                      VL);
  if (Op1.getValueType() != VT)
    Op1 = DAG.getNode(ISD::VP_ZERO_EXTEND, SDLoc(Operands[1]), VT, Op1, Mask,
                      VL);
  ...
  SDValue RM = DAG.getTargetConstant(RISCVVXRndMode::RNU, DL, XLenVT);
  SDValue Policy = DAG.getTargetConstant(RISCVII::TAIL_AGNOSTIC, DL, XLenVT);
  return DAG.getNode(RISCVISD::VAADDU_VL, DL, VT,
                     {Op0, Op1, DAG.getUNDEF(VT), Mask, RM, VL, Policy});
}
#endif // SIFIVE_CUSTOMIZATION
...
///             @llvm.experimental.vector.deinterleave2.nxv64i8(
///               <vscale x 64 x i8> %l)
...
///             @llvm.experimental.vector.interleave2.nxv64i8(
...
bool RISCVTargetLowering::lowerInterleavedScalableStore(
    Instruction *Store, Value *Mask, IntrinsicInst *InterleaveIntrin,
    unsigned Factor) const {
  auto *VPStore = dyn_cast<IntrinsicInst>(Store);
  if (!VPStore || VPStore->getIntrinsicID() != Intrinsic::vp_store)
    return false;

  // Start to build operands for the segmented-store instructions
  ...
  Intrinsic::ID VssegNID = IntrIds[Factor - 2];
