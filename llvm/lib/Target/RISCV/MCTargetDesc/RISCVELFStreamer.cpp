...
bool RISCVELFStreamer::requiresFixups(MCContext &C, const MCExpr *Value,
  ...
  // The __global_pointer$ symbol is generated by ASMPrinter for Compact.
  // And the symbol require MC fixup by R_RISCV_ADD64, R_RISCV_SUB64.
  // Use RISCVELFStreamer::emitValueImpl() to get the MC fixup.
  StringRef CompactGP = "__global_pointer$";
  if (A.getName() == CompactGP || B.getName() == CompactGP)
    return true;
#endif
