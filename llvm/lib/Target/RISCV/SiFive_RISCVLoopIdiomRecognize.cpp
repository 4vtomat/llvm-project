//===- SiFive_RISCVLoopIdiomRecognize.cpp - Loop idiom recognition --------===//
//
...
Value *RISCVLoopIdiomRecognize::expandFindMismatch(
  ...
  // Add LCSSA phis for first and RVVIndexPhi.
  auto *FirstLCSSAPhi = Builder.CreatePHI(First->getType(), 1, "first");
  FirstLCSSAPhi->addIncoming(First, RVVLoopStartBlock);
  ...
  Value *FirstI64 = Builder.CreateZExt(FirstLCSSAPhi, I64Type);
  Value *RVVLoopRes64 = Builder.CreateAdd(RVVIndexLCSSAPhi, FirstI64, "",
                                          /*HasNUW=*/true, /*HasNSW=*/true);
...
void RISCVLoopIdiomRecognize::transformByteCompare(
  ...
  auto FixSuccessorPhis = [&](BasicBlock *SuccBB) {
    for (PHINode &PN : SuccBB->phis()) {
      ...
      bool ResPhi = any_of(PN.incoming_values(), [=](Value *Op) {
        return Op == CmpBB;
      });
