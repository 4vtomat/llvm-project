...
#if SIFIVE_CUSTOMIZATION
STATISTIC(UncountableLoopsVectorized, "Number of uncountable loops vectorized");
#endif
...
public:
  ...
  ~UncountableInnerLoopVectorizer() = default;
...
#if SIFIVE_CUSTOMIZATION
static bool needOtherRVLs(LoopVectorizationLegality &Legal,
                          const LoopVectorizationCostModel &Cost) {
  return !Legal.getFixedOrderRecurrences().empty() ||
         !Legal.getCSAs().empty() ||
  ...
  if (Group->isReverse()) {
    if (Legal->useVLAVectorizer()) {
      ...
      Value *RVL = Builder.CreateZExtOrTrunc(
          State.get(State.RVL, 0, /*NeedsScalar=*/true), Builder.getInt32Ty());
...
#if SIFIVE_CUSTOMIZATION
  Value *RVL =
      State.RVL ? State.get(State.RVL, 0, /*NeedsScalar=*/true) : nullptr;
#endif // SIFIVE_CUSTOMIZATION
  Value *RuntimeVF = nullptr;
  if (VF.isVector()) {
    ...
    if (VF.isVector()) {
      ...
      if (RVL) {
        ...
        Idx = Builder.CreateSub(
            State.get(State.Plan->getPrevRVL(), 0, /*NeedsScalar=*/true),
...
LoopVectorizationCostModel::computeMaxVF(ElementCount UserVF, unsigned UserIC) {
  ...
  if (Legal->isVectorizableUncountable()) {
    FixedScalableVFPair MaxVF =
        computeFeasibleMaxVF(0 /*uncountable*/, UserVF, false);
...
VPValue *VPRecipeBuilder::getOrCreateIV(VPBasicBlock *VPBB, VPlanPtr &Plan) {
  ...
  VPValue *IV = nullptr;
  // TODO: Re-enable this code with proper analysis to avoid inefficient code
  // generation, otherwise there are some lit tests that are failng due to
  // inefficient lowering of the primary induction that is scalarized.
...
VPRecipeBuilder::tryToWidenMemory(Instruction *I, ArrayRef<VPValue *> Operands,
  ...
  if (LoadInst *Load = dyn_cast<LoadInst>(I))
    ...
    return new VPWidenMemoryInstructionRecipe(
        *Load, Ptr, Mask, Consecutive, Reverse, I->getDebugLoc(), Stride,
...
LoopVectorizationPlanner::tryToBuildVPlanWithVPRecipes(VFRange &Range) {
  ...
  if (Legal->useVLAVectorizer() && needOtherRVLs(*Legal, CM)) {
    Plan->createInitRVL();
  }
#endif // SIFIVE_CUSTOMIZATION
...
#if SIFIVE_CUSTOMIZATION
      if (Legal->useVLAVectorizer())
        NewExitingVPV = cast<VPInstruction>(
            Builder.createSelect(Cond, OrigExitingVPV, PhiR, FMFs, DebugLoc(),
...
void VPWidenMemoryInstructionRecipe::execute(VPTransformState &State) {
      ...
      if (isReverse()) {
        if (VPValue *RVL = State.RVL) {
          ...
          Mask = Builder.CreateIntrinsic(
              ...
              {Mask, BlockInMaskPart,
               State.get(RVL, Part, /*NeedsScalar=*/true)});
        } else {
  ...
  if (SI) {
    ...
    for (unsigned Part = 0; Part < State.UF; ++Part) {
      ...
      if (State.EVL && CreateGatherScatter) {
      ...
      if (State.EVL) {
        ...
        if (isReverse()) {
          ...
          if (State.EVL) {
            ...
            StoredVal = Builder.CreateCall(
                VPIntr, {StoredVal, BlockInMaskPart, EVL});
          } else {
#endif // SIFIVE_CUSTOMIZATION
#if SIFIVE_CUSTOMIZATION
            // If we store to reverse consecutive memory locations, then we need
        ...
        if (State.EVL) {
          ...
          NewSI = Builder.CreateCall(
              VPIntr, {StoredVal, VecPtr, BlockInMaskPart, EVL});
  ...
  for (unsigned Part = 0; Part < State.UF; ++Part) {
    ...
    if (State.EVL) {
      ...
      if (State.EVL) {
        ...
        Value *EVL = State.get(State.EVL, VPIteration(0, 0));
        NewLI =
            Builder.CreateCall(VPIntr, {VecPtr, BlockInMaskPart, EVL},
      ...
      if (Reverse) {
        ...
        if (State.EVL) {
          ...
          Value *EVL = State.get(State.EVL, VPIteration(0, 0));
          NewLI = Builder.CreateCall(VPIntr, {NewLI, BlockInMaskPart, EVL});
        } else
#endif // SIFIVE_CUSTOMIZATION
    ...
    if (Speculative) {
      ...
      State.set(State.RVL, VL, Part, /*IsScalar=*/true);
      ...
      State.set(getVPValue(1), VL, Part, /*NeedsScalar=*/true);
    }
    if (!Speculative)
#endif // SIFIVE_CUSTOMIZATION
      ...
      ORE->emit([&]() {
        ...
        if ((Arch == Triple::riscv32 || Arch == Triple::riscv64) &&
          ...
          return OptimizationRemark(LV_NAME, "Vectorized", L->getStartLoc(),
                                    L->getHeader())
                 << "vectorized "
                 << (LVL.isVectorizableUncountable() ? "uncountable " : "")
                 << "loop ((lmul, type): "
