...
void UncountableInnerLoopVectorizer::fixupIVUsers(PHINode *OrigPhi,
                                                  const InductionDescriptor &II,
                                                  Value *VectorTripCount,
                                                  BasicBlock *MiddleBlock,
                                                  VPTransformState &State) {
  ...
  // would be IV + VFirst.
  if (VectorTripCount) {
    InnerLoopVectorizer::fixupIVUsers(OrigPhi, II, VectorTripCount, MiddleBlock,
                                      State);
    return;
  }
...
VectorizationFactor LoopVectorizationPlanner::selectVectorizationFactor() {
  ...
  VPTypeAnalysis TypeInfo(WidestType);
#endif
  ...
  for (auto &P : VPlans) {
    for (ElementCount VF : P->vectorFactors()) {
      ...
      if (UseVPlanCostModel) {
        ...
        VPCostContext CostCtx(TTI, *TLI, Legal->getWidestInductionType(), CM, CM.CostKind);
