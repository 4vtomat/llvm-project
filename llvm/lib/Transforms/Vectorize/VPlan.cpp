...
#if SIFIVE_CUSTOMIZATION
static void moveStepComputationsToIVUpdate(VPTransformState *State,
                                           VPRecipeBase &R, Instruction *Inc) {
  ...
  auto MatchRVLPlaceholder = [&State](Instruction *I) -> Value * {
    Value *V;
    if (match(I, m_SExt(m_Value(V))) || match(I, m_ZExt(m_Value(V))) ||
        match(I, m_Trunc(m_Value(V))) || match(I, m_UIToFP(m_Value(V))))
      return V;
    return I == State->RVLPlaceholder ? I : nullptr;
  };

  SmallVector<Instruction *> InstructionsToMove;
  Instruction *I = nullptr;
  if (isa<VPWidenPointerInductionRecipe>(&R)) {
    // FIXME: Remove with a proper representation of
    // VPWidenPointerInductionRecipe, but for now pattern-match code
    // generated by VPWidenPointerInductionRecipe::execute and move it
    // closer to the update to make sure RVL is available.
    GetElementPtrInst *GEP = dyn_cast<GetElementPtrInst>(Inc);
    assert(GEP &&
           "GEP instruction was not generated for pointer induction update");
    I = cast<Instruction>(GEP->getOperand(GEP->getNumOperands() - 1));
    assert(I->getOpcode() == Instruction::Mul &&
           "Multiplication is expected");
    InstructionsToMove.push_back(I);
    I = cast<Instruction>(I->getOperand(1));
    assert(I->getOpcode() == Instruction::Mul &&
           "Multiplication is expected");
    InstructionsToMove.push_back(I);
    I = cast<Instruction>(I->getOperand(0));
  } else if (isa<VPWidenIntOrFpInductionRecipe>(&R)) {
    // FIXME: Remove with a proper representation of
    // VPWidenIntOrFpInductionRecipe, but for now pattern-match code
    // generated by VPWidenIntOrFpInductionRecipe::execute and move it
    // closer to the update to make sure RVL is available.
    IntrinsicInst *VPAdd = dyn_cast<IntrinsicInst>(Inc);
    assert(VPAdd && "Updated of the IV is expected to be done via intrinsic");
    Value *ShuffleV = VPAdd->getOperand(1);
    Value *InsertV;
    Value *SplatV = nullptr;
    if (VPAdd->getOperand(3)->getType() != State->RVLPlaceholder->getType()) {
        I = cast<Instruction>(VPAdd->getOperand(3));
        Value *V = MatchRVLPlaceholder(I);
        assert((V && V == State->RVLPlaceholder) &&
               "EVL parameter of a vp intrinsic does not use RVL Placeholder");
        InstructionsToMove.push_back(I);
    }

    if (match(ShuffleV,
              m_Shuffle(m_Value(InsertV), m_Value(), m_ZeroMask()))) {
      InstructionsToMove.push_back(cast<Instruction>(ShuffleV));
      if (match(InsertV,
                m_InsertElt(m_Value(), m_Value(SplatV), m_ZeroInt())))
        InstructionsToMove.push_back(cast<Instruction>(InsertV));
    }
    assert(SplatV && "Broadcast was not generated by the IV update");
    I = cast<Instruction>(SplatV);
    assert((I->getOpcode() == Instruction::Mul ||
  ...
  Value *V = MatchRVLPlaceholder(I);
  assert((V && V == State->RVLPlaceholder) &&
         "Multiplication does not use RVL Placeholder");
  InstructionsToMove.push_back(I);
...
void VPlan::execute(VPTransformState *State) {
  ...
  if (Value *RVLPlaceholder = State->RVLPlaceholder) {
    ...
    RVL = State->Builder.CreateZExtOrTrunc(RVL, RVLPlaceholder->getType());
    RVLPlaceholder->replaceAllUsesWith(RVL);
    cast<Instruction>(RVLPlaceholder)->eraseFromParent();
  }
#endif // SIFIVE_CUSTOMIZATION
