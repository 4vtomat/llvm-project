...
; CHECK-NEXT:   experimental-smwg                - 'Smwg' (The Smwg extension adds the mlwid CSR, which is an M-mode read-write CSR, whose least-significant bits set the WID to be used by lower-privilege modes.).
; CHECK-NEXT:   experimental-smwgd               - 'Smwgd' (The Smwgd extension adds the mwiddeleg M-mode read-write CSR. The mwiddeleg register represents a set of WIDs as a bit vector with WID i represented by bit i of the register.).
; CHECK-NEXT:   experimental-ssctr               - 'Ssctr' (Control Transfer Records Supervisor Level).
; CHECK-NEXT:   experimental-sswg                - 'Sswg' (The Sswg extension adds the [H]S-mode read-write slwid CSR, which sets the WID used for modes lower than [H]S-mode.).
...
; CHECK-NEXT:   experimental-zjid                - 'Zjid' (Instruction/Data Cache Synchronization).
...
; CHECK-NEXT:   experimental-zvfofp8min          - 'Zvfofp8min' (OFP8 conversion extension Zvfofp8min).
...
; CHECK-NEXT:   fullfp16                         - fullfp16 (A corresponding feature for NEON fullfp16.).
; CHECK-NEXT:   fuse-arith-eqz                   - Enable fusiong arithmetic with seqz/snez.
; CHECK-NEXT:   fuse-bitfield-extract            - CPU fuses SLLI with SRLI/SRAI to extract bitfield.
; CHECK-NEXT:   fuse-indexed-load                - Enable fusing ADD with load.
; CHECK-NEXT:   fuse-lui-load                    - Enable fusing LUI/AUIPIC with load.
; CHECK-NEXT:   fuse-zba-load                    - Enable fusing Zba instructions with load.
; CHECK-NEXT:   h                                - 'H' (Hypervisor).
; CHECK-NEXT:   has-fast-gather                  - Support fast gather for small vl.
; CHECK-NEXT:   has-fast-large-gather            - Support fast gather for LMUL >= 1.
...
; CHECK-NEXT:   sifive-cheetah                   - SiFive Cheetah processors.
; CHECK-NEXT:   sifive-leopard                   - SiFive Leopard processors.
; CHECK-NEXT:   sifive-lion                      - SiFive Lion processors.
; CHECK-NEXT:   sifive-p400                      - SiFive P400-Series processors.
; CHECK-NEXT:   sifive-p500                      - SiFive P500-Series processors.
; CHECK-NEXT:   sifive-p600                      - SiFive P600-Series processors.
; CHECK-NEXT:   sifive-p800                      - SiFive P800-Series processors.
; CHECK-NEXT:   sifive6                          - SiFive 6-Series processors.
; CHECK-NEXT:   sifive7                          - SiFive 7-Series processors.
; CHECK-NEXT:   slow-vector-fp64                 - Has expensive cost for zve64d.
...
; CHECK-NEXT:   ss                               - 'Ss' (Supervisor Architecture).
...
; CHECK-NEXT:   use-fixed-point                  - Fixed point instructions should be used.
...
; CHECK-NEXT:   zvkb0p1                          - 'Zvkb' (Vector Bit-manipulation used in Cryptography).
; CHECK-NEXT:   zvkg                             - 'Zvkg' (Vector GCM instructions for Cryptography).
; CHECK-NEXT:   zvkg0p1                          - 'Zvkg' (Vector GCM instructions for Cryptography).
...
; CHECK-NEXT:   zvknha0p1                        - 'Zvknha' (Vector SHA-2. (SHA-256 only)).
; CHECK-NEXT:   zvknhb                           - 'Zvknhb' (Vector SHA-2 (SHA-256 and SHA-512)).
; CHECK-NEXT:   zvknhb0p1                        - 'Zvknhb' (Vector SHA-2. (SHA-256 and SHA-512)).
; CHECK-NEXT:   zvkns                            - 'Zvkns' (Vector AES Encryption & Decryption (Single Round)).
...
; CHECK-NEXT:   zvksed0p1                        - 'Zvksed' (SM4 Block Cipher Instructions).
; CHECK-NEXT:   zvksg                            - 'Zvksg' (shorthand for 'Zvks' and 'Zvkg').
; CHECK-NEXT:   zvksh                            - 'Zvksh' (SM3 Hash Function Instructions).
; CHECK-NEXT:   zvksh0p1                         - 'Zvksh' (SM3 Hash Function Instructions).
