...
; RUN: sed 's/iXLen/i32/g' %s | llc -mtriple=riscv32 -mattr=+v -riscv-enable-vl-optimizer=false \
; RUN:   -verify-machineinstrs | FileCheck %s -check-prefixes=CHECK,NOVLOPT
; RUN: sed 's/iXLen/i64/g' %s | llc -mtriple=riscv64 -mattr=+v -riscv-enable-vl-optimizer=false \
; RUN:   -verify-machineinstrs | FileCheck %s -check-prefixes=CHECK,NOVLOPT
