; NOTE: Assertions have been autogenerated by utils/update_llc_test_checks.py
; RUN: llc -mtriple=riscv32 -mattr=+v,+m,+zfh,+zvfh -target-abi=ilp32d \
; RUN:   -verify-machineinstrs < %s | FileCheck %s \
; RUN:   --check-prefixes=CHECK,ZVFH,ZVFH32
; RUN: llc -mtriple=riscv64 -mattr=+v,+m,+zfh,+zvfh -target-abi=lp64d \
; RUN:   -verify-machineinstrs < %s | FileCheck %s \
; RUN:   --check-prefixes=CHECK,ZVFH,ZVFH64
; RUN: llc -mtriple=riscv32 -mattr=+v,+m,+zfh,+zvfhmin -target-abi=ilp32d \
; RUN:   -verify-machineinstrs < %s | FileCheck %s --check-prefixes=CHECK,ZVFHMIN,ZVFHMIN32
; RUN: llc -mtriple=riscv64 -mattr=+v,+m,+zfh,+zvfhmin -target-abi=lp64d \
; RUN:   -verify-machineinstrs < %s | FileCheck %s \
; RUN:   --check-prefixes=CHECK,ZVFHMIN,ZVFHMIN64

declare <7 x i1> @llvm.vp.fcmp.v7f16(<7 x half>, <7 x half>, metadata, <7 x i1>, i32)

define <7 x i1> @fcmp_oeq_vv_v7f16(<7 x half> %va, <7 x half> %vb, <7 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_oeq_vv_v7f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfeq.vv v0, v8, v9, v0.t
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_oeq_vv_v7f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v9
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v12, v8
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfeq.vv v0, v12, v10, v0.t
; ZVFHMIN-NEXT:    ret
  %v = call <7 x i1> @llvm.vp.fcmp.v7f16(<7 x half> %va, <7 x half> %vb, metadata !"oeq", <7 x i1> %m, i32 %evl)
  ret <7 x i1> %v
}

declare <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half>, <8 x half>, metadata, <8 x i1>, i32)

define <8 x i1> @fcmp_oeq_vv_v8f16(<8 x half> %va, <8 x half> %vb, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_oeq_vv_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfeq.vv v0, v8, v9, v0.t
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_oeq_vv_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v9
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v12, v8
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfeq.vv v0, v12, v10, v0.t
; ZVFHMIN-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"oeq", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_oeq_vf_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_oeq_vf_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfeq.vf v0, v8, fa0, v0.t
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_oeq_vf_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfeq.vv v0, v10, v8, v0.t
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"oeq", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_oeq_vf_swap_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_oeq_vf_swap_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfeq.vf v0, v8, fa0, v0.t
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_oeq_vf_swap_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfeq.vv v0, v8, v10, v0.t
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %vb, <8 x half> %va, metadata !"oeq", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ogt_vv_v8f16(<8 x half> %va, <8 x half> %vb, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_ogt_vv_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmflt.vv v0, v9, v8, v0.t
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_ogt_vv_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmflt.vv v0, v8, v10, v0.t
; ZVFHMIN-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"ogt", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ogt_vf_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_ogt_vf_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfgt.vf v0, v8, fa0, v0.t
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_ogt_vf_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmflt.vv v0, v8, v10, v0.t
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"ogt", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ogt_vf_swap_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_ogt_vf_swap_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmflt.vf v0, v8, fa0, v0.t
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_ogt_vf_swap_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmflt.vv v0, v10, v8, v0.t
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %vb, <8 x half> %va, metadata !"ogt", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_oge_vv_v8f16(<8 x half> %va, <8 x half> %vb, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_oge_vv_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfle.vv v0, v9, v8, v0.t
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_oge_vv_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfle.vv v0, v8, v10, v0.t
; ZVFHMIN-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"oge", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_oge_vf_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_oge_vf_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfge.vf v0, v8, fa0, v0.t
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_oge_vf_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfle.vv v0, v8, v10, v0.t
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"oge", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_oge_vf_swap_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_oge_vf_swap_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfle.vf v0, v8, fa0, v0.t
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_oge_vf_swap_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfle.vv v0, v10, v8, v0.t
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %vb, <8 x half> %va, metadata !"oge", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_olt_vv_v8f16(<8 x half> %va, <8 x half> %vb, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_olt_vv_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmflt.vv v0, v8, v9, v0.t
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_olt_vv_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v9
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v12, v8
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmflt.vv v0, v12, v10, v0.t
; ZVFHMIN-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"olt", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_olt_vf_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_olt_vf_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmflt.vf v0, v8, fa0, v0.t
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_olt_vf_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmflt.vv v0, v10, v8, v0.t
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"olt", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_olt_vf_swap_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_olt_vf_swap_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfgt.vf v0, v8, fa0, v0.t
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_olt_vf_swap_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmflt.vv v0, v8, v10, v0.t
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %vb, <8 x half> %va, metadata !"olt", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ole_vv_v8f16(<8 x half> %va, <8 x half> %vb, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_ole_vv_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfle.vv v0, v8, v9, v0.t
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_ole_vv_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v9
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v12, v8
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfle.vv v0, v12, v10, v0.t
; ZVFHMIN-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"ole", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ole_vf_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_ole_vf_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfle.vf v0, v8, fa0, v0.t
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_ole_vf_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfle.vv v0, v10, v8, v0.t
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"ole", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ole_vf_swap_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_ole_vf_swap_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfge.vf v0, v8, fa0, v0.t
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_ole_vf_swap_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfle.vv v0, v8, v10, v0.t
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %vb, <8 x half> %va, metadata !"ole", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_one_vv_v8f16(<8 x half> %va, <8 x half> %vb, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_one_vv_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmflt.vv v10, v8, v9, v0.t
; ZVFH-NEXT:    vmflt.vv v8, v9, v8, v0.t
; ZVFH-NEXT:    vmor.mm v0, v8, v10
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_one_vv_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v9
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v12, v8
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmflt.vv v8, v12, v10, v0.t
; ZVFHMIN-NEXT:    vmflt.vv v9, v10, v12, v0.t
; ZVFHMIN-NEXT:    vmor.mm v0, v9, v8
; ZVFHMIN-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"one", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_one_vf_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_one_vf_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmflt.vf v9, v8, fa0, v0.t
; ZVFH-NEXT:    vmfgt.vf v8, v8, fa0, v0.t
; ZVFH-NEXT:    vmor.mm v0, v8, v9
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_one_vf_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmflt.vv v12, v10, v8, v0.t
; ZVFHMIN-NEXT:    vmflt.vv v8, v8, v10, v0.t
; ZVFHMIN-NEXT:    vmor.mm v0, v8, v12
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"one", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_one_vf_swap_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_one_vf_swap_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfgt.vf v9, v8, fa0, v0.t
; ZVFH-NEXT:    vmflt.vf v8, v8, fa0, v0.t
; ZVFH-NEXT:    vmor.mm v0, v8, v9
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_one_vf_swap_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmflt.vv v12, v8, v10, v0.t
; ZVFHMIN-NEXT:    vmflt.vv v8, v10, v8, v0.t
; ZVFHMIN-NEXT:    vmor.mm v0, v8, v12
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %vb, <8 x half> %va, metadata !"one", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ord_vv_v8f16(<8 x half> %va, <8 x half> %vb, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_ord_vv_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfeq.vv v9, v9, v9, v0.t
; ZVFH-NEXT:    vmfeq.vv v8, v8, v8, v0.t
; ZVFH-NEXT:    vmand.mm v0, v8, v9
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_ord_vv_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfeq.vv v9, v10, v10, v0.t
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfeq.vv v8, v10, v10, v0.t
; ZVFHMIN-NEXT:    vmand.mm v0, v8, v9
; ZVFHMIN-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"ord", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ord_vf_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_ord_vf_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFH-NEXT:    vfmv.v.f v9, fa0
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfeq.vf v9, v9, fa0, v0.t
; ZVFH-NEXT:    vmfeq.vv v8, v8, v8, v0.t
; ZVFH-NEXT:    vmand.mm v0, v8, v9
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_ord_vf_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfeq.vv v8, v10, v10, v0.t
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfeq.vv v9, v10, v10, v0.t
; ZVFHMIN-NEXT:    vmand.mm v0, v8, v9
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"ord", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ord_vf_swap_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_ord_vf_swap_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFH-NEXT:    vfmv.v.f v9, fa0
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfeq.vf v9, v9, fa0, v0.t
; ZVFH-NEXT:    vmfeq.vv v8, v8, v8, v0.t
; ZVFH-NEXT:    vmand.mm v0, v9, v8
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_ord_vf_swap_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfeq.vv v8, v10, v10, v0.t
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfeq.vv v9, v10, v10, v0.t
; ZVFHMIN-NEXT:    vmand.mm v0, v9, v8
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %vb, <8 x half> %va, metadata !"ord", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ueq_vv_v8f16(<8 x half> %va, <8 x half> %vb, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_ueq_vv_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmflt.vv v10, v8, v9, v0.t
; ZVFH-NEXT:    vmflt.vv v8, v9, v8, v0.t
; ZVFH-NEXT:    vmnor.mm v0, v8, v10
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_ueq_vv_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v9
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v12, v8
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmflt.vv v8, v12, v10, v0.t
; ZVFHMIN-NEXT:    vmflt.vv v9, v10, v12, v0.t
; ZVFHMIN-NEXT:    vmnor.mm v0, v9, v8
; ZVFHMIN-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"ueq", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ueq_vf_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_ueq_vf_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmflt.vf v9, v8, fa0, v0.t
; ZVFH-NEXT:    vmfgt.vf v8, v8, fa0, v0.t
; ZVFH-NEXT:    vmnor.mm v0, v8, v9
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_ueq_vf_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmflt.vv v12, v10, v8, v0.t
; ZVFHMIN-NEXT:    vmflt.vv v8, v8, v10, v0.t
; ZVFHMIN-NEXT:    vmnor.mm v0, v8, v12
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"ueq", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ueq_vf_swap_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_ueq_vf_swap_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfgt.vf v9, v8, fa0, v0.t
; ZVFH-NEXT:    vmflt.vf v8, v8, fa0, v0.t
; ZVFH-NEXT:    vmnor.mm v0, v8, v9
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_ueq_vf_swap_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmflt.vv v12, v8, v10, v0.t
; ZVFHMIN-NEXT:    vmflt.vv v8, v10, v8, v0.t
; ZVFHMIN-NEXT:    vmnor.mm v0, v8, v12
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %vb, <8 x half> %va, metadata !"ueq", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ugt_vv_v8f16(<8 x half> %va, <8 x half> %vb, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_ugt_vv_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfle.vv v8, v8, v9, v0.t
; ZVFH-NEXT:    vmnot.m v0, v8
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_ugt_vv_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v9
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v12, v8
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfle.vv v8, v12, v10, v0.t
; ZVFHMIN-NEXT:    vmnot.m v0, v8
; ZVFHMIN-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"ugt", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ugt_vf_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_ugt_vf_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfle.vf v8, v8, fa0, v0.t
; ZVFH-NEXT:    vmnot.m v0, v8
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_ugt_vf_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfle.vv v8, v10, v8, v0.t
; ZVFHMIN-NEXT:    vmnot.m v0, v8
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"ugt", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ugt_vf_swap_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_ugt_vf_swap_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfge.vf v8, v8, fa0, v0.t
; ZVFH-NEXT:    vmnot.m v0, v8
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_ugt_vf_swap_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfle.vv v8, v8, v10, v0.t
; ZVFHMIN-NEXT:    vmnot.m v0, v8
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %vb, <8 x half> %va, metadata !"ugt", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_uge_vv_v8f16(<8 x half> %va, <8 x half> %vb, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_uge_vv_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmflt.vv v8, v8, v9, v0.t
; ZVFH-NEXT:    vmnot.m v0, v8
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_uge_vv_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v9
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v12, v8
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmflt.vv v8, v12, v10, v0.t
; ZVFHMIN-NEXT:    vmnot.m v0, v8
; ZVFHMIN-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"uge", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_uge_vf_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_uge_vf_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmflt.vf v8, v8, fa0, v0.t
; ZVFH-NEXT:    vmnot.m v0, v8
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_uge_vf_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmflt.vv v8, v10, v8, v0.t
; ZVFHMIN-NEXT:    vmnot.m v0, v8
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"uge", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_uge_vf_swap_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_uge_vf_swap_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfgt.vf v8, v8, fa0, v0.t
; ZVFH-NEXT:    vmnot.m v0, v8
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_uge_vf_swap_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmflt.vv v8, v8, v10, v0.t
; ZVFHMIN-NEXT:    vmnot.m v0, v8
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %vb, <8 x half> %va, metadata !"uge", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ult_vv_v8f16(<8 x half> %va, <8 x half> %vb, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_ult_vv_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfle.vv v8, v9, v8, v0.t
; ZVFH-NEXT:    vmnot.m v0, v8
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_ult_vv_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfle.vv v8, v8, v10, v0.t
; ZVFHMIN-NEXT:    vmnot.m v0, v8
; ZVFHMIN-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"ult", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ult_vf_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_ult_vf_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfge.vf v8, v8, fa0, v0.t
; ZVFH-NEXT:    vmnot.m v0, v8
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_ult_vf_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfle.vv v8, v8, v10, v0.t
; ZVFHMIN-NEXT:    vmnot.m v0, v8
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"ult", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ult_vf_swap_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_ult_vf_swap_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfle.vf v8, v8, fa0, v0.t
; ZVFH-NEXT:    vmnot.m v0, v8
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_ult_vf_swap_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfle.vv v8, v10, v8, v0.t
; ZVFHMIN-NEXT:    vmnot.m v0, v8
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %vb, <8 x half> %va, metadata !"ult", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ule_vv_v8f16(<8 x half> %va, <8 x half> %vb, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_ule_vv_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmflt.vv v8, v9, v8, v0.t
; ZVFH-NEXT:    vmnot.m v0, v8
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_ule_vv_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmflt.vv v8, v8, v10, v0.t
; ZVFHMIN-NEXT:    vmnot.m v0, v8
; ZVFHMIN-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"ule", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ule_vf_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_ule_vf_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfgt.vf v8, v8, fa0, v0.t
; ZVFH-NEXT:    vmnot.m v0, v8
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_ule_vf_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmflt.vv v8, v8, v10, v0.t
; ZVFHMIN-NEXT:    vmnot.m v0, v8
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"ule", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ule_vf_swap_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_ule_vf_swap_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmflt.vf v8, v8, fa0, v0.t
; ZVFH-NEXT:    vmnot.m v0, v8
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_ule_vf_swap_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmflt.vv v8, v10, v8, v0.t
; ZVFHMIN-NEXT:    vmnot.m v0, v8
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %vb, <8 x half> %va, metadata !"ule", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_une_vv_v8f16(<8 x half> %va, <8 x half> %vb, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_une_vv_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfne.vv v0, v8, v9, v0.t
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_une_vv_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v9
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v12, v8
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfne.vv v0, v12, v10, v0.t
; ZVFHMIN-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"une", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_une_vf_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_une_vf_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfne.vf v0, v8, fa0, v0.t
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_une_vf_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfne.vv v0, v10, v8, v0.t
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"une", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_une_vf_swap_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_une_vf_swap_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfne.vf v0, v8, fa0, v0.t
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_une_vf_swap_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v8, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfne.vv v0, v8, v10, v0.t
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %vb, <8 x half> %va, metadata !"une", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_uno_vv_v8f16(<8 x half> %va, <8 x half> %vb, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_uno_vv_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfne.vv v9, v9, v9, v0.t
; ZVFH-NEXT:    vmfne.vv v8, v8, v8, v0.t
; ZVFH-NEXT:    vmor.mm v0, v8, v9
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_uno_vv_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfne.vv v9, v10, v10, v0.t
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfne.vv v8, v10, v10, v0.t
; ZVFHMIN-NEXT:    vmor.mm v0, v8, v9
; ZVFHMIN-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"uno", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_uno_vf_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_uno_vf_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFH-NEXT:    vfmv.v.f v9, fa0
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfne.vf v9, v9, fa0, v0.t
; ZVFH-NEXT:    vmfne.vv v8, v8, v8, v0.t
; ZVFH-NEXT:    vmor.mm v0, v8, v9
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_uno_vf_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfne.vv v8, v10, v10, v0.t
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfne.vv v9, v10, v10, v0.t
; ZVFHMIN-NEXT:    vmor.mm v0, v8, v9
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %va, <8 x half> %vb, metadata !"uno", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_uno_vf_swap_v8f16(<8 x half> %va, half %b, <8 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_uno_vf_swap_v8f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFH-NEXT:    vfmv.v.f v9, fa0
; ZVFH-NEXT:    vsetvli zero, a0, e16, m1, ta, ma
; ZVFH-NEXT:    vmfne.vf v9, v9, fa0, v0.t
; ZVFH-NEXT:    vmfne.vv v8, v8, v8, v0.t
; ZVFH-NEXT:    vmor.mm v0, v9, v8
; ZVFH-NEXT:    ret
;
; ZVFHMIN-LABEL: fcmp_uno_vf_swap_v8f16:
; ZVFHMIN:       # %bb.0:
; ZVFHMIN-NEXT:    fmv.x.h a1, fa0
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vmv.v.x v9, a1
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v8
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfne.vv v8, v10, v10, v0.t
; ZVFHMIN-NEXT:    vsetivli zero, 8, e16, m1, ta, ma
; ZVFHMIN-NEXT:    vfwcvt.f.f.v v10, v9
; ZVFHMIN-NEXT:    vsetvli zero, a0, e32, m2, ta, ma
; ZVFHMIN-NEXT:    vmfne.vv v9, v10, v10, v0.t
; ZVFHMIN-NEXT:    vmor.mm v0, v9, v8
; ZVFHMIN-NEXT:    ret
  %elt.head = insertelement <8 x half> poison, half %b, i32 0
  %vb = shufflevector <8 x half> %elt.head, <8 x half> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f16(<8 x half> %vb, <8 x half> %va, metadata !"uno", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

declare <128 x i1> @llvm.vp.fcmp.v128f16(<128 x half>, <128 x half>, metadata, <128 x i1>, i32)

define <128 x i1> @fcmp_oeq_vv_v128f16(<128 x half> %va, <128 x half> %vb, <128 x i1> %m, i32 zeroext %evl) {
; ZVFH-LABEL: fcmp_oeq_vv_v128f16:
; ZVFH:       # %bb.0:
; ZVFH-NEXT:    addi sp, sp, -16
; ZVFH-NEXT:    .cfi_def_cfa_offset 16
; ZVFH-NEXT:    csrr a1, vlenb
; ZVFH-NEXT:    slli a1, a1, 3
; ZVFH-NEXT:    sub sp, sp, a1
; ZVFH-NEXT:    .cfi_escape 0x0f, 0x0d, 0x72, 0x00, 0x11, 0x10, 0x22, 0x11, 0x08, 0x92, 0xa2, 0x38, 0x00, 0x1e, 0x22 # sp + 16 + 8 * vlenb
; ZVFH-NEXT:    addi a1, a0, 128
; ZVFH-NEXT:    li a3, 64
; ZVFH-NEXT:    vsetvli zero, a3, e16, m8, ta, ma
; ZVFH-NEXT:    vle16.v v24, (a1)
; ZVFH-NEXT:    addi a1, sp, 16
; ZVFH-NEXT:    vs8r.v v24, (a1) # Unknown-size Folded Spill
; ZVFH-NEXT:    vle16.v v24, (a0)
; ZVFH-NEXT:    vsetivli zero, 8, e8, m1, ta, ma
; ZVFH-NEXT:    vslidedown.vi v7, v0, 8
; ZVFH-NEXT:    mv a0, a2
; ZVFH-NEXT:    bltu a2, a3, .LBB43_2
; ZVFH-NEXT:  # %bb.1:
; ZVFH-NEXT:    li a0, 64
; ZVFH-NEXT:  .LBB43_2:
; ZVFH-NEXT:    vsetvli zero, a0, e16, m8, ta, ma
; ZVFH-NEXT:    vmfeq.vv v8, v8, v24, v0.t
; ZVFH-NEXT:    addi a0, a2, -64
; ZVFH-NEXT:    sltu a1, a2, a0
; ZVFH-NEXT:    addi a1, a1, -1
; ZVFH-NEXT:    and a0, a1, a0
; ZVFH-NEXT:    vmv1r.v v0, v7
; ZVFH-NEXT:    addi a1, sp, 16
; ZVFH-NEXT:    vl8r.v v24, (a1) # Unknown-size Folded Reload
; ZVFH-NEXT:    vsetvli zero, a0, e16, m8, ta, ma
; ZVFH-NEXT:    vmfeq.vv v9, v16, v24, v0.t
; ZVFH-NEXT:    vsetivli zero, 16, e8, m1, ta, ma
; ZVFH-NEXT:    vslideup.vi v8, v9, 8
; ZVFH-NEXT:    vmv.v.v v0, v8
; ZVFH-NEXT:    csrr a0, vlenb
; ZVFH-NEXT:    slli a0, a0, 3
; ZVFH-NEXT:    add sp, sp, a0
; ZVFH-NEXT:    addi sp, sp, 16
; ZVFH-NEXT:    ret
;
; ZVFHMIN32-LABEL: fcmp_oeq_vv_v128f16:
; ZVFHMIN32:       # %bb.0:
; ZVFHMIN32-NEXT:    addi sp, sp, -512
; ZVFHMIN32-NEXT:    .cfi_def_cfa_offset 512
; ZVFHMIN32-NEXT:    sw ra, 508(sp) # 4-byte Folded Spill
; ZVFHMIN32-NEXT:    sw s0, 504(sp) # 4-byte Folded Spill
; ZVFHMIN32-NEXT:    .cfi_offset ra, -4
; ZVFHMIN32-NEXT:    .cfi_offset s0, -8
; ZVFHMIN32-NEXT:    addi s0, sp, 512
; ZVFHMIN32-NEXT:    .cfi_def_cfa s0, 0
; ZVFHMIN32-NEXT:    andi sp, sp, -128
; ZVFHMIN32-NEXT:    li a1, 64
; ZVFHMIN32-NEXT:    addi a2, sp, 256
; ZVFHMIN32-NEXT:    vsetvli zero, a1, e16, m8, ta, ma
; ZVFHMIN32-NEXT:    vse16.v v16, (a2)
; ZVFHMIN32-NEXT:    addi a1, sp, 128
; ZVFHMIN32-NEXT:    vse16.v v8, (a1)
; ZVFHMIN32-NEXT:    flh fa5, 254(a0)
; ZVFHMIN32-NEXT:    flh fa4, 382(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 127(sp)
; ZVFHMIN32-NEXT:    flh fa5, 252(a0)
; ZVFHMIN32-NEXT:    flh fa4, 380(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 126(sp)
; ZVFHMIN32-NEXT:    flh fa5, 250(a0)
; ZVFHMIN32-NEXT:    flh fa4, 378(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 125(sp)
; ZVFHMIN32-NEXT:    flh fa5, 248(a0)
; ZVFHMIN32-NEXT:    flh fa4, 376(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 124(sp)
; ZVFHMIN32-NEXT:    flh fa5, 246(a0)
; ZVFHMIN32-NEXT:    flh fa4, 374(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 123(sp)
; ZVFHMIN32-NEXT:    flh fa5, 244(a0)
; ZVFHMIN32-NEXT:    flh fa4, 372(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 122(sp)
; ZVFHMIN32-NEXT:    flh fa5, 242(a0)
; ZVFHMIN32-NEXT:    flh fa4, 370(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 121(sp)
; ZVFHMIN32-NEXT:    flh fa5, 240(a0)
; ZVFHMIN32-NEXT:    flh fa4, 368(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 120(sp)
; ZVFHMIN32-NEXT:    flh fa5, 238(a0)
; ZVFHMIN32-NEXT:    flh fa4, 366(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 119(sp)
; ZVFHMIN32-NEXT:    flh fa5, 236(a0)
; ZVFHMIN32-NEXT:    flh fa4, 364(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 118(sp)
; ZVFHMIN32-NEXT:    flh fa5, 234(a0)
; ZVFHMIN32-NEXT:    flh fa4, 362(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 117(sp)
; ZVFHMIN32-NEXT:    flh fa5, 232(a0)
; ZVFHMIN32-NEXT:    flh fa4, 360(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 116(sp)
; ZVFHMIN32-NEXT:    flh fa5, 230(a0)
; ZVFHMIN32-NEXT:    flh fa4, 358(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 115(sp)
; ZVFHMIN32-NEXT:    flh fa5, 228(a0)
; ZVFHMIN32-NEXT:    flh fa4, 356(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 114(sp)
; ZVFHMIN32-NEXT:    flh fa5, 226(a0)
; ZVFHMIN32-NEXT:    flh fa4, 354(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 113(sp)
; ZVFHMIN32-NEXT:    flh fa5, 224(a0)
; ZVFHMIN32-NEXT:    flh fa4, 352(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 112(sp)
; ZVFHMIN32-NEXT:    flh fa5, 222(a0)
; ZVFHMIN32-NEXT:    flh fa4, 350(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 111(sp)
; ZVFHMIN32-NEXT:    flh fa5, 126(a0)
; ZVFHMIN32-NEXT:    flh fa4, 254(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 63(sp)
; ZVFHMIN32-NEXT:    flh fa5, 124(a0)
; ZVFHMIN32-NEXT:    flh fa4, 252(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 62(sp)
; ZVFHMIN32-NEXT:    flh fa5, 122(a0)
; ZVFHMIN32-NEXT:    flh fa4, 250(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 61(sp)
; ZVFHMIN32-NEXT:    flh fa5, 120(a0)
; ZVFHMIN32-NEXT:    flh fa4, 248(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 60(sp)
; ZVFHMIN32-NEXT:    flh fa5, 118(a0)
; ZVFHMIN32-NEXT:    flh fa4, 246(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 59(sp)
; ZVFHMIN32-NEXT:    flh fa5, 116(a0)
; ZVFHMIN32-NEXT:    flh fa4, 244(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 58(sp)
; ZVFHMIN32-NEXT:    flh fa5, 114(a0)
; ZVFHMIN32-NEXT:    flh fa4, 242(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 57(sp)
; ZVFHMIN32-NEXT:    flh fa5, 112(a0)
; ZVFHMIN32-NEXT:    flh fa4, 240(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 56(sp)
; ZVFHMIN32-NEXT:    flh fa5, 110(a0)
; ZVFHMIN32-NEXT:    flh fa4, 238(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 55(sp)
; ZVFHMIN32-NEXT:    flh fa5, 108(a0)
; ZVFHMIN32-NEXT:    flh fa4, 236(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 54(sp)
; ZVFHMIN32-NEXT:    flh fa5, 106(a0)
; ZVFHMIN32-NEXT:    flh fa4, 234(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 53(sp)
; ZVFHMIN32-NEXT:    flh fa5, 104(a0)
; ZVFHMIN32-NEXT:    flh fa4, 232(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 52(sp)
; ZVFHMIN32-NEXT:    flh fa5, 102(a0)
; ZVFHMIN32-NEXT:    flh fa4, 230(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 51(sp)
; ZVFHMIN32-NEXT:    flh fa5, 100(a0)
; ZVFHMIN32-NEXT:    flh fa4, 228(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 50(sp)
; ZVFHMIN32-NEXT:    flh fa5, 98(a0)
; ZVFHMIN32-NEXT:    flh fa4, 226(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 49(sp)
; ZVFHMIN32-NEXT:    flh fa5, 96(a0)
; ZVFHMIN32-NEXT:    flh fa4, 224(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 48(sp)
; ZVFHMIN32-NEXT:    flh fa5, 94(a0)
; ZVFHMIN32-NEXT:    flh fa4, 222(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 47(sp)
; ZVFHMIN32-NEXT:    flh fa5, 220(a0)
; ZVFHMIN32-NEXT:    flh fa4, 348(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 110(sp)
; ZVFHMIN32-NEXT:    flh fa5, 218(a0)
; ZVFHMIN32-NEXT:    flh fa4, 346(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 109(sp)
; ZVFHMIN32-NEXT:    flh fa5, 216(a0)
; ZVFHMIN32-NEXT:    flh fa4, 344(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 108(sp)
; ZVFHMIN32-NEXT:    flh fa5, 214(a0)
; ZVFHMIN32-NEXT:    flh fa4, 342(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 107(sp)
; ZVFHMIN32-NEXT:    flh fa5, 212(a0)
; ZVFHMIN32-NEXT:    flh fa4, 340(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 106(sp)
; ZVFHMIN32-NEXT:    flh fa5, 210(a0)
; ZVFHMIN32-NEXT:    flh fa4, 338(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 105(sp)
; ZVFHMIN32-NEXT:    flh fa5, 208(a0)
; ZVFHMIN32-NEXT:    flh fa4, 336(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 104(sp)
; ZVFHMIN32-NEXT:    flh fa5, 206(a0)
; ZVFHMIN32-NEXT:    flh fa4, 334(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 103(sp)
; ZVFHMIN32-NEXT:    flh fa5, 204(a0)
; ZVFHMIN32-NEXT:    flh fa4, 332(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 102(sp)
; ZVFHMIN32-NEXT:    flh fa5, 202(a0)
; ZVFHMIN32-NEXT:    flh fa4, 330(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 101(sp)
; ZVFHMIN32-NEXT:    flh fa5, 200(a0)
; ZVFHMIN32-NEXT:    flh fa4, 328(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 100(sp)
; ZVFHMIN32-NEXT:    flh fa5, 198(a0)
; ZVFHMIN32-NEXT:    flh fa4, 326(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 99(sp)
; ZVFHMIN32-NEXT:    flh fa5, 196(a0)
; ZVFHMIN32-NEXT:    flh fa4, 324(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 98(sp)
; ZVFHMIN32-NEXT:    flh fa5, 194(a0)
; ZVFHMIN32-NEXT:    flh fa4, 322(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 97(sp)
; ZVFHMIN32-NEXT:    flh fa5, 192(a0)
; ZVFHMIN32-NEXT:    flh fa4, 320(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 96(sp)
; ZVFHMIN32-NEXT:    flh fa5, 190(a0)
; ZVFHMIN32-NEXT:    flh fa4, 318(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 95(sp)
; ZVFHMIN32-NEXT:    flh fa5, 188(a0)
; ZVFHMIN32-NEXT:    flh fa4, 316(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 94(sp)
; ZVFHMIN32-NEXT:    flh fa5, 92(a0)
; ZVFHMIN32-NEXT:    flh fa4, 220(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 46(sp)
; ZVFHMIN32-NEXT:    flh fa5, 90(a0)
; ZVFHMIN32-NEXT:    flh fa4, 218(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 45(sp)
; ZVFHMIN32-NEXT:    flh fa5, 88(a0)
; ZVFHMIN32-NEXT:    flh fa4, 216(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 44(sp)
; ZVFHMIN32-NEXT:    flh fa5, 86(a0)
; ZVFHMIN32-NEXT:    flh fa4, 214(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 43(sp)
; ZVFHMIN32-NEXT:    flh fa5, 84(a0)
; ZVFHMIN32-NEXT:    flh fa4, 212(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 42(sp)
; ZVFHMIN32-NEXT:    flh fa5, 82(a0)
; ZVFHMIN32-NEXT:    flh fa4, 210(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 41(sp)
; ZVFHMIN32-NEXT:    flh fa5, 80(a0)
; ZVFHMIN32-NEXT:    flh fa4, 208(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 40(sp)
; ZVFHMIN32-NEXT:    flh fa5, 78(a0)
; ZVFHMIN32-NEXT:    flh fa4, 206(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 39(sp)
; ZVFHMIN32-NEXT:    flh fa5, 76(a0)
; ZVFHMIN32-NEXT:    flh fa4, 204(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 38(sp)
; ZVFHMIN32-NEXT:    flh fa5, 74(a0)
; ZVFHMIN32-NEXT:    flh fa4, 202(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 37(sp)
; ZVFHMIN32-NEXT:    flh fa5, 72(a0)
; ZVFHMIN32-NEXT:    flh fa4, 200(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 36(sp)
; ZVFHMIN32-NEXT:    flh fa5, 70(a0)
; ZVFHMIN32-NEXT:    flh fa4, 198(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 35(sp)
; ZVFHMIN32-NEXT:    flh fa5, 68(a0)
; ZVFHMIN32-NEXT:    flh fa4, 196(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 34(sp)
; ZVFHMIN32-NEXT:    flh fa5, 66(a0)
; ZVFHMIN32-NEXT:    flh fa4, 194(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 33(sp)
; ZVFHMIN32-NEXT:    flh fa5, 64(a0)
; ZVFHMIN32-NEXT:    flh fa4, 192(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 32(sp)
; ZVFHMIN32-NEXT:    flh fa5, 62(a0)
; ZVFHMIN32-NEXT:    flh fa4, 190(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 31(sp)
; ZVFHMIN32-NEXT:    flh fa5, 60(a0)
; ZVFHMIN32-NEXT:    flh fa4, 188(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 30(sp)
; ZVFHMIN32-NEXT:    flh fa5, 186(a0)
; ZVFHMIN32-NEXT:    flh fa4, 314(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 93(sp)
; ZVFHMIN32-NEXT:    flh fa5, 184(a0)
; ZVFHMIN32-NEXT:    flh fa4, 312(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 92(sp)
; ZVFHMIN32-NEXT:    flh fa5, 182(a0)
; ZVFHMIN32-NEXT:    flh fa4, 310(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 91(sp)
; ZVFHMIN32-NEXT:    flh fa5, 180(a0)
; ZVFHMIN32-NEXT:    flh fa4, 308(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 90(sp)
; ZVFHMIN32-NEXT:    flh fa5, 178(a0)
; ZVFHMIN32-NEXT:    flh fa4, 306(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 89(sp)
; ZVFHMIN32-NEXT:    flh fa5, 176(a0)
; ZVFHMIN32-NEXT:    flh fa4, 304(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 88(sp)
; ZVFHMIN32-NEXT:    flh fa5, 174(a0)
; ZVFHMIN32-NEXT:    flh fa4, 302(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 87(sp)
; ZVFHMIN32-NEXT:    flh fa5, 172(a0)
; ZVFHMIN32-NEXT:    flh fa4, 300(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 86(sp)
; ZVFHMIN32-NEXT:    flh fa5, 170(a0)
; ZVFHMIN32-NEXT:    flh fa4, 298(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 85(sp)
; ZVFHMIN32-NEXT:    flh fa5, 168(a0)
; ZVFHMIN32-NEXT:    flh fa4, 296(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 84(sp)
; ZVFHMIN32-NEXT:    flh fa5, 166(a0)
; ZVFHMIN32-NEXT:    flh fa4, 294(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 83(sp)
; ZVFHMIN32-NEXT:    flh fa5, 164(a0)
; ZVFHMIN32-NEXT:    flh fa4, 292(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 82(sp)
; ZVFHMIN32-NEXT:    flh fa5, 162(a0)
; ZVFHMIN32-NEXT:    flh fa4, 290(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 81(sp)
; ZVFHMIN32-NEXT:    flh fa5, 160(a0)
; ZVFHMIN32-NEXT:    flh fa4, 288(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 80(sp)
; ZVFHMIN32-NEXT:    flh fa5, 158(a0)
; ZVFHMIN32-NEXT:    flh fa4, 286(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 79(sp)
; ZVFHMIN32-NEXT:    flh fa5, 156(a0)
; ZVFHMIN32-NEXT:    flh fa4, 284(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 78(sp)
; ZVFHMIN32-NEXT:    flh fa5, 154(a0)
; ZVFHMIN32-NEXT:    flh fa4, 282(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 77(sp)
; ZVFHMIN32-NEXT:    flh fa5, 58(a0)
; ZVFHMIN32-NEXT:    flh fa4, 186(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 29(sp)
; ZVFHMIN32-NEXT:    flh fa5, 56(a0)
; ZVFHMIN32-NEXT:    flh fa4, 184(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 28(sp)
; ZVFHMIN32-NEXT:    flh fa5, 54(a0)
; ZVFHMIN32-NEXT:    flh fa4, 182(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 27(sp)
; ZVFHMIN32-NEXT:    flh fa5, 52(a0)
; ZVFHMIN32-NEXT:    flh fa4, 180(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 26(sp)
; ZVFHMIN32-NEXT:    flh fa5, 50(a0)
; ZVFHMIN32-NEXT:    flh fa4, 178(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 25(sp)
; ZVFHMIN32-NEXT:    flh fa5, 48(a0)
; ZVFHMIN32-NEXT:    flh fa4, 176(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 24(sp)
; ZVFHMIN32-NEXT:    flh fa5, 46(a0)
; ZVFHMIN32-NEXT:    flh fa4, 174(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 23(sp)
; ZVFHMIN32-NEXT:    flh fa5, 44(a0)
; ZVFHMIN32-NEXT:    flh fa4, 172(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 22(sp)
; ZVFHMIN32-NEXT:    flh fa5, 42(a0)
; ZVFHMIN32-NEXT:    flh fa4, 170(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 21(sp)
; ZVFHMIN32-NEXT:    flh fa5, 40(a0)
; ZVFHMIN32-NEXT:    flh fa4, 168(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 20(sp)
; ZVFHMIN32-NEXT:    flh fa5, 38(a0)
; ZVFHMIN32-NEXT:    flh fa4, 166(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 19(sp)
; ZVFHMIN32-NEXT:    flh fa5, 36(a0)
; ZVFHMIN32-NEXT:    flh fa4, 164(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 18(sp)
; ZVFHMIN32-NEXT:    flh fa5, 34(a0)
; ZVFHMIN32-NEXT:    flh fa4, 162(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 17(sp)
; ZVFHMIN32-NEXT:    flh fa5, 32(a0)
; ZVFHMIN32-NEXT:    flh fa4, 160(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 16(sp)
; ZVFHMIN32-NEXT:    flh fa5, 30(a0)
; ZVFHMIN32-NEXT:    flh fa4, 158(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 15(sp)
; ZVFHMIN32-NEXT:    flh fa5, 28(a0)
; ZVFHMIN32-NEXT:    flh fa4, 156(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 14(sp)
; ZVFHMIN32-NEXT:    flh fa5, 26(a0)
; ZVFHMIN32-NEXT:    flh fa4, 154(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 13(sp)
; ZVFHMIN32-NEXT:    flh fa5, 152(a0)
; ZVFHMIN32-NEXT:    flh fa4, 280(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 76(sp)
; ZVFHMIN32-NEXT:    flh fa5, 150(a0)
; ZVFHMIN32-NEXT:    flh fa4, 278(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 75(sp)
; ZVFHMIN32-NEXT:    flh fa5, 148(a0)
; ZVFHMIN32-NEXT:    flh fa4, 276(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 74(sp)
; ZVFHMIN32-NEXT:    flh fa5, 146(a0)
; ZVFHMIN32-NEXT:    flh fa4, 274(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 73(sp)
; ZVFHMIN32-NEXT:    flh fa5, 144(a0)
; ZVFHMIN32-NEXT:    flh fa4, 272(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 72(sp)
; ZVFHMIN32-NEXT:    flh fa5, 142(a0)
; ZVFHMIN32-NEXT:    flh fa4, 270(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 71(sp)
; ZVFHMIN32-NEXT:    flh fa5, 140(a0)
; ZVFHMIN32-NEXT:    flh fa4, 268(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 70(sp)
; ZVFHMIN32-NEXT:    flh fa5, 138(a0)
; ZVFHMIN32-NEXT:    flh fa4, 266(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 69(sp)
; ZVFHMIN32-NEXT:    flh fa5, 136(a0)
; ZVFHMIN32-NEXT:    flh fa4, 264(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 68(sp)
; ZVFHMIN32-NEXT:    flh fa5, 134(a0)
; ZVFHMIN32-NEXT:    flh fa4, 262(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 67(sp)
; ZVFHMIN32-NEXT:    flh fa5, 132(a0)
; ZVFHMIN32-NEXT:    flh fa4, 260(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 66(sp)
; ZVFHMIN32-NEXT:    flh fa5, 130(a0)
; ZVFHMIN32-NEXT:    flh fa4, 258(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 65(sp)
; ZVFHMIN32-NEXT:    flh fa5, 128(a0)
; ZVFHMIN32-NEXT:    flh fa4, 256(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 64(sp)
; ZVFHMIN32-NEXT:    flh fa5, 24(a0)
; ZVFHMIN32-NEXT:    flh fa4, 152(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 12(sp)
; ZVFHMIN32-NEXT:    flh fa5, 22(a0)
; ZVFHMIN32-NEXT:    flh fa4, 150(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 11(sp)
; ZVFHMIN32-NEXT:    flh fa5, 20(a0)
; ZVFHMIN32-NEXT:    flh fa4, 148(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 10(sp)
; ZVFHMIN32-NEXT:    flh fa5, 18(a0)
; ZVFHMIN32-NEXT:    flh fa4, 146(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 9(sp)
; ZVFHMIN32-NEXT:    flh fa5, 16(a0)
; ZVFHMIN32-NEXT:    flh fa4, 144(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 8(sp)
; ZVFHMIN32-NEXT:    flh fa5, 14(a0)
; ZVFHMIN32-NEXT:    flh fa4, 142(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 7(sp)
; ZVFHMIN32-NEXT:    flh fa5, 12(a0)
; ZVFHMIN32-NEXT:    flh fa4, 140(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 6(sp)
; ZVFHMIN32-NEXT:    flh fa5, 10(a0)
; ZVFHMIN32-NEXT:    flh fa4, 138(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 5(sp)
; ZVFHMIN32-NEXT:    flh fa5, 8(a0)
; ZVFHMIN32-NEXT:    flh fa4, 136(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 4(sp)
; ZVFHMIN32-NEXT:    flh fa5, 6(a0)
; ZVFHMIN32-NEXT:    flh fa4, 134(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 3(sp)
; ZVFHMIN32-NEXT:    flh fa5, 4(a0)
; ZVFHMIN32-NEXT:    flh fa4, 132(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 2(sp)
; ZVFHMIN32-NEXT:    flh fa5, 2(a0)
; ZVFHMIN32-NEXT:    flh fa4, 130(sp)
; ZVFHMIN32-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN32-NEXT:    sb a1, 1(sp)
; ZVFHMIN32-NEXT:    flh fa5, 0(a0)
; ZVFHMIN32-NEXT:    flh fa4, 128(sp)
; ZVFHMIN32-NEXT:    feq.h a0, fa4, fa5
; ZVFHMIN32-NEXT:    sb a0, 0(sp)
; ZVFHMIN32-NEXT:    li a0, 128
; ZVFHMIN32-NEXT:    mv a1, sp
; ZVFHMIN32-NEXT:    vsetvli zero, a0, e8, m8, ta, ma
; ZVFHMIN32-NEXT:    vle8.v v8, (a1)
; ZVFHMIN32-NEXT:    vand.vi v8, v8, 1
; ZVFHMIN32-NEXT:    vmsne.vi v0, v8, 0
; ZVFHMIN32-NEXT:    addi sp, s0, -512
; ZVFHMIN32-NEXT:    lw ra, 508(sp) # 4-byte Folded Reload
; ZVFHMIN32-NEXT:    lw s0, 504(sp) # 4-byte Folded Reload
; ZVFHMIN32-NEXT:    addi sp, sp, 512
; ZVFHMIN32-NEXT:    ret
;
; ZVFHMIN64-LABEL: fcmp_oeq_vv_v128f16:
; ZVFHMIN64:       # %bb.0:
; ZVFHMIN64-NEXT:    addi sp, sp, -512
; ZVFHMIN64-NEXT:    .cfi_def_cfa_offset 512
; ZVFHMIN64-NEXT:    sd ra, 504(sp) # 8-byte Folded Spill
; ZVFHMIN64-NEXT:    sd s0, 496(sp) # 8-byte Folded Spill
; ZVFHMIN64-NEXT:    .cfi_offset ra, -8
; ZVFHMIN64-NEXT:    .cfi_offset s0, -16
; ZVFHMIN64-NEXT:    addi s0, sp, 512
; ZVFHMIN64-NEXT:    .cfi_def_cfa s0, 0
; ZVFHMIN64-NEXT:    andi sp, sp, -128
; ZVFHMIN64-NEXT:    li a1, 64
; ZVFHMIN64-NEXT:    addi a2, sp, 256
; ZVFHMIN64-NEXT:    vsetvli zero, a1, e16, m8, ta, ma
; ZVFHMIN64-NEXT:    vse16.v v16, (a2)
; ZVFHMIN64-NEXT:    addi a1, sp, 128
; ZVFHMIN64-NEXT:    vse16.v v8, (a1)
; ZVFHMIN64-NEXT:    flh fa5, 254(a0)
; ZVFHMIN64-NEXT:    flh fa4, 382(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 127(sp)
; ZVFHMIN64-NEXT:    flh fa5, 252(a0)
; ZVFHMIN64-NEXT:    flh fa4, 380(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 126(sp)
; ZVFHMIN64-NEXT:    flh fa5, 250(a0)
; ZVFHMIN64-NEXT:    flh fa4, 378(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 125(sp)
; ZVFHMIN64-NEXT:    flh fa5, 248(a0)
; ZVFHMIN64-NEXT:    flh fa4, 376(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 124(sp)
; ZVFHMIN64-NEXT:    flh fa5, 246(a0)
; ZVFHMIN64-NEXT:    flh fa4, 374(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 123(sp)
; ZVFHMIN64-NEXT:    flh fa5, 244(a0)
; ZVFHMIN64-NEXT:    flh fa4, 372(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 122(sp)
; ZVFHMIN64-NEXT:    flh fa5, 242(a0)
; ZVFHMIN64-NEXT:    flh fa4, 370(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 121(sp)
; ZVFHMIN64-NEXT:    flh fa5, 240(a0)
; ZVFHMIN64-NEXT:    flh fa4, 368(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 120(sp)
; ZVFHMIN64-NEXT:    flh fa5, 238(a0)
; ZVFHMIN64-NEXT:    flh fa4, 366(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 119(sp)
; ZVFHMIN64-NEXT:    flh fa5, 236(a0)
; ZVFHMIN64-NEXT:    flh fa4, 364(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 118(sp)
; ZVFHMIN64-NEXT:    flh fa5, 234(a0)
; ZVFHMIN64-NEXT:    flh fa4, 362(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 117(sp)
; ZVFHMIN64-NEXT:    flh fa5, 232(a0)
; ZVFHMIN64-NEXT:    flh fa4, 360(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 116(sp)
; ZVFHMIN64-NEXT:    flh fa5, 230(a0)
; ZVFHMIN64-NEXT:    flh fa4, 358(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 115(sp)
; ZVFHMIN64-NEXT:    flh fa5, 228(a0)
; ZVFHMIN64-NEXT:    flh fa4, 356(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 114(sp)
; ZVFHMIN64-NEXT:    flh fa5, 226(a0)
; ZVFHMIN64-NEXT:    flh fa4, 354(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 113(sp)
; ZVFHMIN64-NEXT:    flh fa5, 224(a0)
; ZVFHMIN64-NEXT:    flh fa4, 352(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 112(sp)
; ZVFHMIN64-NEXT:    flh fa5, 222(a0)
; ZVFHMIN64-NEXT:    flh fa4, 350(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 111(sp)
; ZVFHMIN64-NEXT:    flh fa5, 126(a0)
; ZVFHMIN64-NEXT:    flh fa4, 254(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 63(sp)
; ZVFHMIN64-NEXT:    flh fa5, 124(a0)
; ZVFHMIN64-NEXT:    flh fa4, 252(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 62(sp)
; ZVFHMIN64-NEXT:    flh fa5, 122(a0)
; ZVFHMIN64-NEXT:    flh fa4, 250(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 61(sp)
; ZVFHMIN64-NEXT:    flh fa5, 120(a0)
; ZVFHMIN64-NEXT:    flh fa4, 248(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 60(sp)
; ZVFHMIN64-NEXT:    flh fa5, 118(a0)
; ZVFHMIN64-NEXT:    flh fa4, 246(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 59(sp)
; ZVFHMIN64-NEXT:    flh fa5, 116(a0)
; ZVFHMIN64-NEXT:    flh fa4, 244(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 58(sp)
; ZVFHMIN64-NEXT:    flh fa5, 114(a0)
; ZVFHMIN64-NEXT:    flh fa4, 242(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 57(sp)
; ZVFHMIN64-NEXT:    flh fa5, 112(a0)
; ZVFHMIN64-NEXT:    flh fa4, 240(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 56(sp)
; ZVFHMIN64-NEXT:    flh fa5, 110(a0)
; ZVFHMIN64-NEXT:    flh fa4, 238(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 55(sp)
; ZVFHMIN64-NEXT:    flh fa5, 108(a0)
; ZVFHMIN64-NEXT:    flh fa4, 236(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 54(sp)
; ZVFHMIN64-NEXT:    flh fa5, 106(a0)
; ZVFHMIN64-NEXT:    flh fa4, 234(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 53(sp)
; ZVFHMIN64-NEXT:    flh fa5, 104(a0)
; ZVFHMIN64-NEXT:    flh fa4, 232(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 52(sp)
; ZVFHMIN64-NEXT:    flh fa5, 102(a0)
; ZVFHMIN64-NEXT:    flh fa4, 230(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 51(sp)
; ZVFHMIN64-NEXT:    flh fa5, 100(a0)
; ZVFHMIN64-NEXT:    flh fa4, 228(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 50(sp)
; ZVFHMIN64-NEXT:    flh fa5, 98(a0)
; ZVFHMIN64-NEXT:    flh fa4, 226(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 49(sp)
; ZVFHMIN64-NEXT:    flh fa5, 96(a0)
; ZVFHMIN64-NEXT:    flh fa4, 224(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 48(sp)
; ZVFHMIN64-NEXT:    flh fa5, 94(a0)
; ZVFHMIN64-NEXT:    flh fa4, 222(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 47(sp)
; ZVFHMIN64-NEXT:    flh fa5, 220(a0)
; ZVFHMIN64-NEXT:    flh fa4, 348(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 110(sp)
; ZVFHMIN64-NEXT:    flh fa5, 218(a0)
; ZVFHMIN64-NEXT:    flh fa4, 346(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 109(sp)
; ZVFHMIN64-NEXT:    flh fa5, 216(a0)
; ZVFHMIN64-NEXT:    flh fa4, 344(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 108(sp)
; ZVFHMIN64-NEXT:    flh fa5, 214(a0)
; ZVFHMIN64-NEXT:    flh fa4, 342(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 107(sp)
; ZVFHMIN64-NEXT:    flh fa5, 212(a0)
; ZVFHMIN64-NEXT:    flh fa4, 340(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 106(sp)
; ZVFHMIN64-NEXT:    flh fa5, 210(a0)
; ZVFHMIN64-NEXT:    flh fa4, 338(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 105(sp)
; ZVFHMIN64-NEXT:    flh fa5, 208(a0)
; ZVFHMIN64-NEXT:    flh fa4, 336(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 104(sp)
; ZVFHMIN64-NEXT:    flh fa5, 206(a0)
; ZVFHMIN64-NEXT:    flh fa4, 334(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 103(sp)
; ZVFHMIN64-NEXT:    flh fa5, 204(a0)
; ZVFHMIN64-NEXT:    flh fa4, 332(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 102(sp)
; ZVFHMIN64-NEXT:    flh fa5, 202(a0)
; ZVFHMIN64-NEXT:    flh fa4, 330(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 101(sp)
; ZVFHMIN64-NEXT:    flh fa5, 200(a0)
; ZVFHMIN64-NEXT:    flh fa4, 328(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 100(sp)
; ZVFHMIN64-NEXT:    flh fa5, 198(a0)
; ZVFHMIN64-NEXT:    flh fa4, 326(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 99(sp)
; ZVFHMIN64-NEXT:    flh fa5, 196(a0)
; ZVFHMIN64-NEXT:    flh fa4, 324(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 98(sp)
; ZVFHMIN64-NEXT:    flh fa5, 194(a0)
; ZVFHMIN64-NEXT:    flh fa4, 322(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 97(sp)
; ZVFHMIN64-NEXT:    flh fa5, 192(a0)
; ZVFHMIN64-NEXT:    flh fa4, 320(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 96(sp)
; ZVFHMIN64-NEXT:    flh fa5, 190(a0)
; ZVFHMIN64-NEXT:    flh fa4, 318(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 95(sp)
; ZVFHMIN64-NEXT:    flh fa5, 188(a0)
; ZVFHMIN64-NEXT:    flh fa4, 316(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 94(sp)
; ZVFHMIN64-NEXT:    flh fa5, 92(a0)
; ZVFHMIN64-NEXT:    flh fa4, 220(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 46(sp)
; ZVFHMIN64-NEXT:    flh fa5, 90(a0)
; ZVFHMIN64-NEXT:    flh fa4, 218(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 45(sp)
; ZVFHMIN64-NEXT:    flh fa5, 88(a0)
; ZVFHMIN64-NEXT:    flh fa4, 216(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 44(sp)
; ZVFHMIN64-NEXT:    flh fa5, 86(a0)
; ZVFHMIN64-NEXT:    flh fa4, 214(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 43(sp)
; ZVFHMIN64-NEXT:    flh fa5, 84(a0)
; ZVFHMIN64-NEXT:    flh fa4, 212(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 42(sp)
; ZVFHMIN64-NEXT:    flh fa5, 82(a0)
; ZVFHMIN64-NEXT:    flh fa4, 210(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 41(sp)
; ZVFHMIN64-NEXT:    flh fa5, 80(a0)
; ZVFHMIN64-NEXT:    flh fa4, 208(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 40(sp)
; ZVFHMIN64-NEXT:    flh fa5, 78(a0)
; ZVFHMIN64-NEXT:    flh fa4, 206(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 39(sp)
; ZVFHMIN64-NEXT:    flh fa5, 76(a0)
; ZVFHMIN64-NEXT:    flh fa4, 204(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 38(sp)
; ZVFHMIN64-NEXT:    flh fa5, 74(a0)
; ZVFHMIN64-NEXT:    flh fa4, 202(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 37(sp)
; ZVFHMIN64-NEXT:    flh fa5, 72(a0)
; ZVFHMIN64-NEXT:    flh fa4, 200(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 36(sp)
; ZVFHMIN64-NEXT:    flh fa5, 70(a0)
; ZVFHMIN64-NEXT:    flh fa4, 198(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 35(sp)
; ZVFHMIN64-NEXT:    flh fa5, 68(a0)
; ZVFHMIN64-NEXT:    flh fa4, 196(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 34(sp)
; ZVFHMIN64-NEXT:    flh fa5, 66(a0)
; ZVFHMIN64-NEXT:    flh fa4, 194(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 33(sp)
; ZVFHMIN64-NEXT:    flh fa5, 64(a0)
; ZVFHMIN64-NEXT:    flh fa4, 192(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 32(sp)
; ZVFHMIN64-NEXT:    flh fa5, 62(a0)
; ZVFHMIN64-NEXT:    flh fa4, 190(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 31(sp)
; ZVFHMIN64-NEXT:    flh fa5, 60(a0)
; ZVFHMIN64-NEXT:    flh fa4, 188(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 30(sp)
; ZVFHMIN64-NEXT:    flh fa5, 186(a0)
; ZVFHMIN64-NEXT:    flh fa4, 314(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 93(sp)
; ZVFHMIN64-NEXT:    flh fa5, 184(a0)
; ZVFHMIN64-NEXT:    flh fa4, 312(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 92(sp)
; ZVFHMIN64-NEXT:    flh fa5, 182(a0)
; ZVFHMIN64-NEXT:    flh fa4, 310(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 91(sp)
; ZVFHMIN64-NEXT:    flh fa5, 180(a0)
; ZVFHMIN64-NEXT:    flh fa4, 308(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 90(sp)
; ZVFHMIN64-NEXT:    flh fa5, 178(a0)
; ZVFHMIN64-NEXT:    flh fa4, 306(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 89(sp)
; ZVFHMIN64-NEXT:    flh fa5, 176(a0)
; ZVFHMIN64-NEXT:    flh fa4, 304(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 88(sp)
; ZVFHMIN64-NEXT:    flh fa5, 174(a0)
; ZVFHMIN64-NEXT:    flh fa4, 302(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 87(sp)
; ZVFHMIN64-NEXT:    flh fa5, 172(a0)
; ZVFHMIN64-NEXT:    flh fa4, 300(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 86(sp)
; ZVFHMIN64-NEXT:    flh fa5, 170(a0)
; ZVFHMIN64-NEXT:    flh fa4, 298(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 85(sp)
; ZVFHMIN64-NEXT:    flh fa5, 168(a0)
; ZVFHMIN64-NEXT:    flh fa4, 296(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 84(sp)
; ZVFHMIN64-NEXT:    flh fa5, 166(a0)
; ZVFHMIN64-NEXT:    flh fa4, 294(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 83(sp)
; ZVFHMIN64-NEXT:    flh fa5, 164(a0)
; ZVFHMIN64-NEXT:    flh fa4, 292(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 82(sp)
; ZVFHMIN64-NEXT:    flh fa5, 162(a0)
; ZVFHMIN64-NEXT:    flh fa4, 290(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 81(sp)
; ZVFHMIN64-NEXT:    flh fa5, 160(a0)
; ZVFHMIN64-NEXT:    flh fa4, 288(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 80(sp)
; ZVFHMIN64-NEXT:    flh fa5, 158(a0)
; ZVFHMIN64-NEXT:    flh fa4, 286(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 79(sp)
; ZVFHMIN64-NEXT:    flh fa5, 156(a0)
; ZVFHMIN64-NEXT:    flh fa4, 284(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 78(sp)
; ZVFHMIN64-NEXT:    flh fa5, 154(a0)
; ZVFHMIN64-NEXT:    flh fa4, 282(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 77(sp)
; ZVFHMIN64-NEXT:    flh fa5, 58(a0)
; ZVFHMIN64-NEXT:    flh fa4, 186(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 29(sp)
; ZVFHMIN64-NEXT:    flh fa5, 56(a0)
; ZVFHMIN64-NEXT:    flh fa4, 184(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 28(sp)
; ZVFHMIN64-NEXT:    flh fa5, 54(a0)
; ZVFHMIN64-NEXT:    flh fa4, 182(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 27(sp)
; ZVFHMIN64-NEXT:    flh fa5, 52(a0)
; ZVFHMIN64-NEXT:    flh fa4, 180(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 26(sp)
; ZVFHMIN64-NEXT:    flh fa5, 50(a0)
; ZVFHMIN64-NEXT:    flh fa4, 178(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 25(sp)
; ZVFHMIN64-NEXT:    flh fa5, 48(a0)
; ZVFHMIN64-NEXT:    flh fa4, 176(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 24(sp)
; ZVFHMIN64-NEXT:    flh fa5, 46(a0)
; ZVFHMIN64-NEXT:    flh fa4, 174(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 23(sp)
; ZVFHMIN64-NEXT:    flh fa5, 44(a0)
; ZVFHMIN64-NEXT:    flh fa4, 172(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 22(sp)
; ZVFHMIN64-NEXT:    flh fa5, 42(a0)
; ZVFHMIN64-NEXT:    flh fa4, 170(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 21(sp)
; ZVFHMIN64-NEXT:    flh fa5, 40(a0)
; ZVFHMIN64-NEXT:    flh fa4, 168(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 20(sp)
; ZVFHMIN64-NEXT:    flh fa5, 38(a0)
; ZVFHMIN64-NEXT:    flh fa4, 166(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 19(sp)
; ZVFHMIN64-NEXT:    flh fa5, 36(a0)
; ZVFHMIN64-NEXT:    flh fa4, 164(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 18(sp)
; ZVFHMIN64-NEXT:    flh fa5, 34(a0)
; ZVFHMIN64-NEXT:    flh fa4, 162(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 17(sp)
; ZVFHMIN64-NEXT:    flh fa5, 32(a0)
; ZVFHMIN64-NEXT:    flh fa4, 160(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 16(sp)
; ZVFHMIN64-NEXT:    flh fa5, 30(a0)
; ZVFHMIN64-NEXT:    flh fa4, 158(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 15(sp)
; ZVFHMIN64-NEXT:    flh fa5, 28(a0)
; ZVFHMIN64-NEXT:    flh fa4, 156(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 14(sp)
; ZVFHMIN64-NEXT:    flh fa5, 26(a0)
; ZVFHMIN64-NEXT:    flh fa4, 154(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 13(sp)
; ZVFHMIN64-NEXT:    flh fa5, 152(a0)
; ZVFHMIN64-NEXT:    flh fa4, 280(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 76(sp)
; ZVFHMIN64-NEXT:    flh fa5, 150(a0)
; ZVFHMIN64-NEXT:    flh fa4, 278(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 75(sp)
; ZVFHMIN64-NEXT:    flh fa5, 148(a0)
; ZVFHMIN64-NEXT:    flh fa4, 276(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 74(sp)
; ZVFHMIN64-NEXT:    flh fa5, 146(a0)
; ZVFHMIN64-NEXT:    flh fa4, 274(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 73(sp)
; ZVFHMIN64-NEXT:    flh fa5, 144(a0)
; ZVFHMIN64-NEXT:    flh fa4, 272(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 72(sp)
; ZVFHMIN64-NEXT:    flh fa5, 142(a0)
; ZVFHMIN64-NEXT:    flh fa4, 270(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 71(sp)
; ZVFHMIN64-NEXT:    flh fa5, 140(a0)
; ZVFHMIN64-NEXT:    flh fa4, 268(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 70(sp)
; ZVFHMIN64-NEXT:    flh fa5, 138(a0)
; ZVFHMIN64-NEXT:    flh fa4, 266(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 69(sp)
; ZVFHMIN64-NEXT:    flh fa5, 136(a0)
; ZVFHMIN64-NEXT:    flh fa4, 264(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 68(sp)
; ZVFHMIN64-NEXT:    flh fa5, 134(a0)
; ZVFHMIN64-NEXT:    flh fa4, 262(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 67(sp)
; ZVFHMIN64-NEXT:    flh fa5, 132(a0)
; ZVFHMIN64-NEXT:    flh fa4, 260(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 66(sp)
; ZVFHMIN64-NEXT:    flh fa5, 130(a0)
; ZVFHMIN64-NEXT:    flh fa4, 258(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 65(sp)
; ZVFHMIN64-NEXT:    flh fa5, 128(a0)
; ZVFHMIN64-NEXT:    flh fa4, 256(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 64(sp)
; ZVFHMIN64-NEXT:    flh fa5, 24(a0)
; ZVFHMIN64-NEXT:    flh fa4, 152(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 12(sp)
; ZVFHMIN64-NEXT:    flh fa5, 22(a0)
; ZVFHMIN64-NEXT:    flh fa4, 150(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 11(sp)
; ZVFHMIN64-NEXT:    flh fa5, 20(a0)
; ZVFHMIN64-NEXT:    flh fa4, 148(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 10(sp)
; ZVFHMIN64-NEXT:    flh fa5, 18(a0)
; ZVFHMIN64-NEXT:    flh fa4, 146(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 9(sp)
; ZVFHMIN64-NEXT:    flh fa5, 16(a0)
; ZVFHMIN64-NEXT:    flh fa4, 144(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 8(sp)
; ZVFHMIN64-NEXT:    flh fa5, 14(a0)
; ZVFHMIN64-NEXT:    flh fa4, 142(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 7(sp)
; ZVFHMIN64-NEXT:    flh fa5, 12(a0)
; ZVFHMIN64-NEXT:    flh fa4, 140(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 6(sp)
; ZVFHMIN64-NEXT:    flh fa5, 10(a0)
; ZVFHMIN64-NEXT:    flh fa4, 138(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 5(sp)
; ZVFHMIN64-NEXT:    flh fa5, 8(a0)
; ZVFHMIN64-NEXT:    flh fa4, 136(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 4(sp)
; ZVFHMIN64-NEXT:    flh fa5, 6(a0)
; ZVFHMIN64-NEXT:    flh fa4, 134(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 3(sp)
; ZVFHMIN64-NEXT:    flh fa5, 4(a0)
; ZVFHMIN64-NEXT:    flh fa4, 132(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 2(sp)
; ZVFHMIN64-NEXT:    flh fa5, 2(a0)
; ZVFHMIN64-NEXT:    flh fa4, 130(sp)
; ZVFHMIN64-NEXT:    feq.h a1, fa4, fa5
; ZVFHMIN64-NEXT:    sb a1, 1(sp)
; ZVFHMIN64-NEXT:    flh fa5, 0(a0)
; ZVFHMIN64-NEXT:    flh fa4, 128(sp)
; ZVFHMIN64-NEXT:    feq.h a0, fa4, fa5
; ZVFHMIN64-NEXT:    sb a0, 0(sp)
; ZVFHMIN64-NEXT:    li a0, 128
; ZVFHMIN64-NEXT:    mv a1, sp
; ZVFHMIN64-NEXT:    vsetvli zero, a0, e8, m8, ta, ma
; ZVFHMIN64-NEXT:    vle8.v v8, (a1)
; ZVFHMIN64-NEXT:    vand.vi v8, v8, 1
; ZVFHMIN64-NEXT:    vmsne.vi v0, v8, 0
; ZVFHMIN64-NEXT:    addi sp, s0, -512
; ZVFHMIN64-NEXT:    ld ra, 504(sp) # 8-byte Folded Reload
; ZVFHMIN64-NEXT:    ld s0, 496(sp) # 8-byte Folded Reload
; ZVFHMIN64-NEXT:    addi sp, sp, 512
; ZVFHMIN64-NEXT:    ret
  %v = call <128 x i1> @llvm.vp.fcmp.v128f16(<128 x half> %va, <128 x half> %vb, metadata !"oeq", <128 x i1> %m, i32 %evl)
  ret <128 x i1> %v
}

declare <7 x i1> @llvm.vp.fcmp.v7f64(<7 x double>, <7 x double>, metadata, <7 x i1>, i32)

define <7 x i1> @fcmp_oeq_vv_v7f64(<7 x double> %va, <7 x double> %vb, <7 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_oeq_vv_v7f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfeq.vv v0, v8, v12, v0.t
; CHECK-NEXT:    ret
  %v = call <7 x i1> @llvm.vp.fcmp.v7f64(<7 x double> %va, <7 x double> %vb, metadata !"oeq", <7 x i1> %m, i32 %evl)
  ret <7 x i1> %v
}

declare <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double>, <8 x double>, metadata, <8 x i1>, i32)

define <8 x i1> @fcmp_oeq_vv_v8f64(<8 x double> %va, <8 x double> %vb, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_oeq_vv_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfeq.vv v0, v8, v12, v0.t
; CHECK-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"oeq", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_oeq_vf_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_oeq_vf_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfeq.vf v0, v8, fa0, v0.t
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"oeq", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_oeq_vf_swap_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_oeq_vf_swap_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfeq.vf v0, v8, fa0, v0.t
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %vb, <8 x double> %va, metadata !"oeq", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ogt_vv_v8f64(<8 x double> %va, <8 x double> %vb, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_ogt_vv_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmflt.vv v0, v12, v8, v0.t
; CHECK-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"ogt", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ogt_vf_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_ogt_vf_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfgt.vf v0, v8, fa0, v0.t
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"ogt", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ogt_vf_swap_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_ogt_vf_swap_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmflt.vf v0, v8, fa0, v0.t
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %vb, <8 x double> %va, metadata !"ogt", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_oge_vv_v8f64(<8 x double> %va, <8 x double> %vb, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_oge_vv_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfle.vv v0, v12, v8, v0.t
; CHECK-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"oge", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_oge_vf_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_oge_vf_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfge.vf v0, v8, fa0, v0.t
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"oge", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_oge_vf_swap_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_oge_vf_swap_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfle.vf v0, v8, fa0, v0.t
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %vb, <8 x double> %va, metadata !"oge", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_olt_vv_v8f64(<8 x double> %va, <8 x double> %vb, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_olt_vv_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmflt.vv v0, v8, v12, v0.t
; CHECK-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"olt", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_olt_vf_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_olt_vf_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmflt.vf v0, v8, fa0, v0.t
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"olt", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_olt_vf_swap_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_olt_vf_swap_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfgt.vf v0, v8, fa0, v0.t
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %vb, <8 x double> %va, metadata !"olt", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ole_vv_v8f64(<8 x double> %va, <8 x double> %vb, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_ole_vv_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfle.vv v0, v8, v12, v0.t
; CHECK-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"ole", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ole_vf_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_ole_vf_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfle.vf v0, v8, fa0, v0.t
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"ole", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ole_vf_swap_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_ole_vf_swap_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfge.vf v0, v8, fa0, v0.t
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %vb, <8 x double> %va, metadata !"ole", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_one_vv_v8f64(<8 x double> %va, <8 x double> %vb, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_one_vv_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmflt.vv v16, v8, v12, v0.t
; CHECK-NEXT:    vmflt.vv v8, v12, v8, v0.t
; CHECK-NEXT:    vmor.mm v0, v8, v16
; CHECK-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"one", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_one_vf_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_one_vf_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmflt.vf v12, v8, fa0, v0.t
; CHECK-NEXT:    vmfgt.vf v8, v8, fa0, v0.t
; CHECK-NEXT:    vmor.mm v0, v8, v12
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"one", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_one_vf_swap_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_one_vf_swap_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfgt.vf v12, v8, fa0, v0.t
; CHECK-NEXT:    vmflt.vf v8, v8, fa0, v0.t
; CHECK-NEXT:    vmor.mm v0, v8, v12
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %vb, <8 x double> %va, metadata !"one", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ord_vv_v8f64(<8 x double> %va, <8 x double> %vb, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_ord_vv_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfeq.vv v12, v12, v12, v0.t
; CHECK-NEXT:    vmfeq.vv v8, v8, v8, v0.t
; CHECK-NEXT:    vmand.mm v0, v8, v12
; CHECK-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"ord", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ord_vf_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_ord_vf_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetivli zero, 8, e64, m4, ta, ma
; CHECK-NEXT:    vfmv.v.f v12, fa0
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfeq.vf v12, v12, fa0, v0.t
; CHECK-NEXT:    vmfeq.vv v8, v8, v8, v0.t
; CHECK-NEXT:    vmand.mm v0, v8, v12
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"ord", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ord_vf_swap_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_ord_vf_swap_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetivli zero, 8, e64, m4, ta, ma
; CHECK-NEXT:    vfmv.v.f v12, fa0
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfeq.vf v12, v12, fa0, v0.t
; CHECK-NEXT:    vmfeq.vv v8, v8, v8, v0.t
; CHECK-NEXT:    vmand.mm v0, v12, v8
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %vb, <8 x double> %va, metadata !"ord", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ueq_vv_v8f64(<8 x double> %va, <8 x double> %vb, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_ueq_vv_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmflt.vv v16, v8, v12, v0.t
; CHECK-NEXT:    vmflt.vv v8, v12, v8, v0.t
; CHECK-NEXT:    vmnor.mm v0, v8, v16
; CHECK-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"ueq", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ueq_vf_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_ueq_vf_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmflt.vf v12, v8, fa0, v0.t
; CHECK-NEXT:    vmfgt.vf v8, v8, fa0, v0.t
; CHECK-NEXT:    vmnor.mm v0, v8, v12
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"ueq", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ueq_vf_swap_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_ueq_vf_swap_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfgt.vf v12, v8, fa0, v0.t
; CHECK-NEXT:    vmflt.vf v8, v8, fa0, v0.t
; CHECK-NEXT:    vmnor.mm v0, v8, v12
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %vb, <8 x double> %va, metadata !"ueq", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ugt_vv_v8f64(<8 x double> %va, <8 x double> %vb, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_ugt_vv_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfle.vv v8, v8, v12, v0.t
; CHECK-NEXT:    vmnot.m v0, v8
; CHECK-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"ugt", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ugt_vf_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_ugt_vf_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfle.vf v8, v8, fa0, v0.t
; CHECK-NEXT:    vmnot.m v0, v8
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"ugt", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ugt_vf_swap_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_ugt_vf_swap_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfge.vf v8, v8, fa0, v0.t
; CHECK-NEXT:    vmnot.m v0, v8
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %vb, <8 x double> %va, metadata !"ugt", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_uge_vv_v8f64(<8 x double> %va, <8 x double> %vb, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_uge_vv_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmflt.vv v8, v8, v12, v0.t
; CHECK-NEXT:    vmnot.m v0, v8
; CHECK-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"uge", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_uge_vf_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_uge_vf_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmflt.vf v8, v8, fa0, v0.t
; CHECK-NEXT:    vmnot.m v0, v8
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"uge", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_uge_vf_swap_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_uge_vf_swap_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfgt.vf v8, v8, fa0, v0.t
; CHECK-NEXT:    vmnot.m v0, v8
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %vb, <8 x double> %va, metadata !"uge", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ult_vv_v8f64(<8 x double> %va, <8 x double> %vb, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_ult_vv_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfle.vv v8, v12, v8, v0.t
; CHECK-NEXT:    vmnot.m v0, v8
; CHECK-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"ult", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ult_vf_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_ult_vf_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfge.vf v8, v8, fa0, v0.t
; CHECK-NEXT:    vmnot.m v0, v8
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"ult", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ult_vf_swap_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_ult_vf_swap_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfle.vf v8, v8, fa0, v0.t
; CHECK-NEXT:    vmnot.m v0, v8
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %vb, <8 x double> %va, metadata !"ult", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ule_vv_v8f64(<8 x double> %va, <8 x double> %vb, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_ule_vv_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmflt.vv v8, v12, v8, v0.t
; CHECK-NEXT:    vmnot.m v0, v8
; CHECK-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"ule", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ule_vf_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_ule_vf_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfgt.vf v8, v8, fa0, v0.t
; CHECK-NEXT:    vmnot.m v0, v8
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"ule", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_ule_vf_swap_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_ule_vf_swap_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmflt.vf v8, v8, fa0, v0.t
; CHECK-NEXT:    vmnot.m v0, v8
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %vb, <8 x double> %va, metadata !"ule", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_une_vv_v8f64(<8 x double> %va, <8 x double> %vb, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_une_vv_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfne.vv v0, v8, v12, v0.t
; CHECK-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"une", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_une_vf_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_une_vf_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfne.vf v0, v8, fa0, v0.t
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"une", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_une_vf_swap_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_une_vf_swap_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfne.vf v0, v8, fa0, v0.t
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %vb, <8 x double> %va, metadata !"une", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_uno_vv_v8f64(<8 x double> %va, <8 x double> %vb, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_uno_vv_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfne.vv v12, v12, v12, v0.t
; CHECK-NEXT:    vmfne.vv v8, v8, v8, v0.t
; CHECK-NEXT:    vmor.mm v0, v8, v12
; CHECK-NEXT:    ret
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"uno", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_uno_vf_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_uno_vf_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetivli zero, 8, e64, m4, ta, ma
; CHECK-NEXT:    vfmv.v.f v12, fa0
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfne.vf v12, v12, fa0, v0.t
; CHECK-NEXT:    vmfne.vv v8, v8, v8, v0.t
; CHECK-NEXT:    vmor.mm v0, v8, v12
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %va, <8 x double> %vb, metadata !"uno", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

define <8 x i1> @fcmp_uno_vf_swap_v8f64(<8 x double> %va, double %b, <8 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_uno_vf_swap_v8f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    vsetivli zero, 8, e64, m4, ta, ma
; CHECK-NEXT:    vfmv.v.f v12, fa0
; CHECK-NEXT:    vsetvli zero, a0, e64, m4, ta, ma
; CHECK-NEXT:    vmfne.vf v12, v12, fa0, v0.t
; CHECK-NEXT:    vmfne.vv v8, v8, v8, v0.t
; CHECK-NEXT:    vmor.mm v0, v12, v8
; CHECK-NEXT:    ret
  %elt.head = insertelement <8 x double> poison, double %b, i32 0
  %vb = shufflevector <8 x double> %elt.head, <8 x double> poison, <8 x i32> zeroinitializer
  %v = call <8 x i1> @llvm.vp.fcmp.v8f64(<8 x double> %vb, <8 x double> %va, metadata !"uno", <8 x i1> %m, i32 %evl)
  ret <8 x i1> %v
}

declare <32 x i1> @llvm.vp.fcmp.v32f64(<32 x double>, <32 x double>, metadata, <32 x i1>, i32)

define <32 x i1> @fcmp_oeq_vv_v32f64(<32 x double> %va, <32 x double> %vb, <32 x i1> %m, i32 zeroext %evl) {
; CHECK-LABEL: fcmp_oeq_vv_v32f64:
; CHECK:       # %bb.0:
; CHECK-NEXT:    addi sp, sp, -16
; CHECK-NEXT:    .cfi_def_cfa_offset 16
; CHECK-NEXT:    csrr a1, vlenb
; CHECK-NEXT:    slli a1, a1, 4
; CHECK-NEXT:    sub sp, sp, a1
; CHECK-NEXT:    .cfi_escape 0x0f, 0x0d, 0x72, 0x00, 0x11, 0x10, 0x22, 0x11, 0x10, 0x92, 0xa2, 0x38, 0x00, 0x1e, 0x22 # sp + 16 + 16 * vlenb
; CHECK-NEXT:    addi a1, a0, 128
; CHECK-NEXT:    vsetivli zero, 16, e64, m8, ta, ma
; CHECK-NEXT:    vle64.v v24, (a1)
; CHECK-NEXT:    csrr a1, vlenb
; CHECK-NEXT:    slli a1, a1, 3
; CHECK-NEXT:    add a1, sp, a1
; CHECK-NEXT:    addi a1, a1, 16
; CHECK-NEXT:    vs8r.v v24, (a1) # Unknown-size Folded Spill
; CHECK-NEXT:    vle64.v v24, (a0)
; CHECK-NEXT:    addi a0, sp, 16
; CHECK-NEXT:    vs8r.v v24, (a0) # Unknown-size Folded Spill
; CHECK-NEXT:    li a1, 16
; CHECK-NEXT:    vsetivli zero, 2, e8, mf4, ta, ma
; CHECK-NEXT:    vslidedown.vi v7, v0, 2
; CHECK-NEXT:    mv a0, a2
; CHECK-NEXT:    bltu a2, a1, .LBB87_2
; CHECK-NEXT:  # %bb.1:
; CHECK-NEXT:    li a0, 16
; CHECK-NEXT:  .LBB87_2:
; CHECK-NEXT:    addi a1, sp, 16
; CHECK-NEXT:    vl8r.v v24, (a1) # Unknown-size Folded Reload
; CHECK-NEXT:    vsetvli zero, a0, e64, m8, ta, ma
; CHECK-NEXT:    vmfeq.vv v8, v8, v24, v0.t
; CHECK-NEXT:    addi a0, a2, -16
; CHECK-NEXT:    sltu a1, a2, a0
; CHECK-NEXT:    addi a1, a1, -1
; CHECK-NEXT:    and a0, a1, a0
; CHECK-NEXT:    vmv1r.v v0, v7
; CHECK-NEXT:    csrr a1, vlenb
; CHECK-NEXT:    slli a1, a1, 3
; CHECK-NEXT:    add a1, sp, a1
; CHECK-NEXT:    addi a1, a1, 16
; CHECK-NEXT:    vl8r.v v24, (a1) # Unknown-size Folded Reload
; CHECK-NEXT:    vsetvli zero, a0, e64, m8, ta, ma
; CHECK-NEXT:    vmfeq.vv v9, v16, v24, v0.t
; CHECK-NEXT:    vsetivli zero, 4, e8, mf4, ta, ma
; CHECK-NEXT:    vslideup.vi v8, v9, 2
; CHECK-NEXT:    vmv1r.v v0, v8
; CHECK-NEXT:    csrr a0, vlenb
; CHECK-NEXT:    slli a0, a0, 4
; CHECK-NEXT:    add sp, sp, a0
; CHECK-NEXT:    addi sp, sp, 16
; CHECK-NEXT:    ret
  %v = call <32 x i1> @llvm.vp.fcmp.v32f64(<32 x double> %va, <32 x double> %vb, metadata !"oeq", <32 x i1> %m, i32 %evl)
  ret <32 x i1> %v
}
;; NOTE: These prefixes are unused and the list is autogenerated. Do not add tests below this line:
; ZVFH32: {{.*}}
; ZVFH64: {{.*}}
