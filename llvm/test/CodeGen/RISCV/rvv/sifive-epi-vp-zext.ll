...
; RUN: llc -mtriple=riscv64 -mattr=+m,+f,+d,+c,+v < %s | \
; RUN:     FileCheck %s
...
define <vscale x 16 x i64> @test_vp_zext_nxv16i64_nxv16i16(<vscale x 16 x i16> %a, i32 zeroext %evl) {
...
; CHECK-NEXT:    ret
    ...
    %x = call <vscale x 16 x i64> @llvm.vp.zext.nxv16i64.nxv16i16(<vscale x 16 x i16> %a, <vscale x 16 x i1> %m.splat, i32 %evl)
    ret <vscale x 16 x i64> %x
}
declare <vscale x 16 x i64> @llvm.vp.zext.nxv16i64.nxv16i16(<vscale x 16 x i16> %op, <vscale x 16 x i1> %mask, i32 %evl)
