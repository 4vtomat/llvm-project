...
# RUN: llvm-mc -filetype=obj -triple riscv32 -mattr=+zkr --mattr=+zicsr < %s \
# RUN:     | llvm-objdump -d --mattr=+zkr --mattr=+zicsr - \
# RUN:     | FileCheck -check-prefix=CHECK-INST-ALIAS %s
#
# RUN: llvm-mc %s -triple=riscv64 --mattr=+zicsr -riscv-no-aliases -show-encoding \
# RUN:     | FileCheck -check-prefixes=CHECK-INST,CHECK-ENC %s
# RUN: llvm-mc -filetype=obj -triple riscv64 -mattr=+zkr --mattr=+zicsr < %s \
# RUN:     | llvm-objdump -d --mattr=+zkr --mattr=+zicsr  - \
# RUN:     | FileCheck -check-prefix=CHECK-INST-ALIAS %s
