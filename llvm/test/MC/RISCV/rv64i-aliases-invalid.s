<<<<<<< HEAD
# UNSUPPORTED: windows
# RUN: not llvm-mc -triple=riscv64 --mattr=+zicsr -riscv-no-aliases < %s -o /dev/null 2>&1 | FileCheck %s
# RUN: not llvm-mc -triple=riscv64 --mattr=+zicsr < %s 2>&1 -o /dev/null | FileCheck %s
=======
