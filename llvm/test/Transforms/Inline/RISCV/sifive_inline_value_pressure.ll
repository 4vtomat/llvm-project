; RUN: opt -S -passes=inline -inline-threshold=900  -value-liveness-enable=3 -value-liveness-max-GPR-num=5 -value-liveness-gpr-bypass -debug-only=inline-cost -disable-output < %s 2>&1 | FileCheck %s -check-prefix=CHECK_VLP_FAIL
; RUN: opt -S -passes=inline -inline-threshold=900  -value-liveness-enable=3 -value-liveness-max-GPR-num=10 -value-liveness-gpr-bypass -debug-only=inline-cost -disable-output < %s 2>&1 |  FileCheck %s -check-prefix=CHECK_VLP_SUCCESS
; REQUIRES: asserts
