...
; RUN: opt -p loop-unroll -allow-epilog-peeling -allow-peeling-select -mtriple=riscv64 -S %s | FileCheck %s
...
; CHECK:       [[OUTER_HEADER]]:
; CHECK-NEXT:    [[IV:%.*]] = phi i16 [ 0, %[[ENTRY]] ], [ [[IV_NEXT_LCSSA:%.*]], %[[INNER_LATCH:.*]] ]
...
; CHECK:       [[INNER_LATCH]]:
; CHECK-NEXT:    [[IV_NEXT_LCSSA]] = add nuw nsw i16 [[IV]], 1
; CHECK-NEXT:    [[TMP0:%.*]] = icmp eq i16 [[IV_NEXT_LCSSA]], 999
; CHECK-NEXT:    br i1 [[TMP0]], label %[[EXIT_PEEL_BEGIN:.*]], label %[[OUTER_HEADER]], !llvm.loop [[LOOP0:![0-9]+]]
; CHECK:       [[EXIT_PEEL_BEGIN]]:
; CHECK-NEXT:    [[TMP2:%.*]] = phi i16 [ [[IV_NEXT_LCSSA]], %[[INNER_LATCH]] ]
; CHECK-NEXT:    br label %[[OUTER_HEADER_PEEL:.*]]
; CHECK:       [[OUTER_HEADER_PEEL]]:
; CHECK-NEXT:    [[TMP1:%.*]] = icmp eq i16 [[TMP2]], 999
; CHECK-NEXT:    br label %[[INNER_HEADER_PEEL:.*]]
; CHECK:       [[INNER_HEADER_PEEL]]:
; CHECK-NEXT:    br i1 [[TMP1]], label %[[THEN_PEEL:.*]], label %[[INNER_LATCH_PEEL:.*]]
; CHECK:       [[THEN_PEEL]]:
; CHECK-NEXT:    call void @foo(i32 0)
; CHECK-NEXT:    br label %[[INNER_LATCH_PEEL]]
; CHECK:       [[INNER_LATCH_PEEL]]:
; CHECK-NEXT:    [[IV_NEXT_PEEL:%.*]] = add i16 [[TMP2]], 1
