...
; RUN: opt -p loop-unroll -allow-epilog-peeling -mtriple=riscv64 -scev-cheap-expansion-budget=2 -S %s | FileCheck --check-prefix=BUDGET2 %s
; RUN: opt -p loop-unroll -allow-epilog-peeling -mtriple=riscv64 -scev-cheap-expansion-budget=3 -S %s | FileCheck --check-prefix=BUDGET3 %s
; REQUIRES: riscv-registered-target

; SIFIVE Note: the use of -allow-epilog-peeling -mtriple=riscv64 is required
...
; BUDGET2:       [[LOOP_HEADER]]:
; BUDGET2-NEXT:    [[TMP2:%.*]] = phi i32 [ [[START]], %[[ENTRY]] ], [ [[IV_NEXT_PEEL:%.*]], %[[LOOP_LATCH:.*]] ]
; BUDGET2-NEXT:    [[C_PEEL:%.*]] = icmp eq i32 [[TMP2]], [[SUB]]
; BUDGET2-NEXT:    br i1 [[C_PEEL]], label %[[THEN:.*]], label %[[LOOP_LATCH]]
; BUDGET2:       [[THEN]]:
; BUDGET2-NEXT:    br label %[[LOOP_LATCH]]
; BUDGET2:       [[LOOP_LATCH]]:
; BUDGET2-NEXT:    [[IV_NEXT_PEEL]] = add nsw i32 [[TMP2]], 1
; BUDGET2-NEXT:    [[EC_PEEL:%.*]] = icmp eq i32 [[IV_NEXT_PEEL]], [[END]]
; BUDGET2-NEXT:    br i1 [[EC_PEEL]], label %[[EXIT:.*]], label %[[LOOP_HEADER]]
; BUDGET2:       [[EXIT]]:
...
; BUDGET3-NEXT:    [[PEELCOUNTXSTEP_EXIT:%.*]] = sub i32 [[END]], 1
; BUDGET3-NEXT:    [[EC:%.*]] = icmp eq i32 [[IV_NEXT_PEEL]], [[PEELCOUNTXSTEP_EXIT]]
...
; BUDGET3-NEXT:    [[TMP2:%.*]] = phi i32 [ [[START]], %[[ENTRY]] ], [ [[DOTPH]], %[[EXIT_PEEL_BEGIN_LOOPEXIT]] ]
; BUDGET3-NEXT:    br label %[[LOOP_HEADER_PEEL:.*]]
; BUDGET3:       [[LOOP_HEADER_PEEL]]:
; BUDGET3-NEXT:    [[C_PEEL1:%.*]] = icmp eq i32 [[TMP2]], [[SUB]]
...
; BUDGET3-NEXT:    [[IV_NEXT_PEEL1:%.*]] = add nsw i32 [[TMP2]], 1
