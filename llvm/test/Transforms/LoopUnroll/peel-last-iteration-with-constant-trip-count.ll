...
; RUN: opt -p loop-unroll -allow-epilog-peeling -allow-peeling-select -mtriple=riscv64 -unroll-full-max-count=0 -S %s | FileCheck %s
; REQUIRES: riscv-registered-target

; SIFIVE Note: the switches: -allow-epilog-peeling -allow-peeling-select -mtriple=riscv64
;              are required.
...
; CHECK-NEXT:    br label %[[LOOP:.*]]
; CHECK:       [[LOOP]]:
; CHECK-NEXT:    [[IV:%.*]] = phi i64 [ 0, %[[ENTRY]] ], [ [[IV_NEXT_LCSSA:%.*]], %[[LOOP]] ]
; CHECK-NEXT:    call void @foo(i32 20)
; CHECK-NEXT:    [[IV_NEXT_LCSSA]] = add nuw nsw i64 [[IV]], 1
; CHECK-NEXT:    [[CMP_PEEL1:%.*]] = icmp eq i64 [[IV_NEXT_LCSSA]], 63
; CHECK-NEXT:    br i1 [[CMP_PEEL1]], label %[[EXIT_PEEL_BEGIN:.*]], label %[[LOOP]], !llvm.loop [[LOOP2:![0-9]+]]
; CHECK:       [[EXIT_PEEL_BEGIN]]:
...
; CHECK-NEXT:    br i1 [[EC1]], label %[[LOOP]], label %[[EXIT_PEEL_BEGIN:.*]], !llvm.loop [[LOOP6:![0-9]+]]
; CHECK:       [[EXIT_PEEL_BEGIN]]:
; CHECK-NEXT:    [[IV:%.*]] = phi i64 [ [[IV_NEXT1]], %[[LOOP]] ]
; CHECK-NEXT:    br label %[[LOOP_PEEL:.*]]
; CHECK:       [[LOOP_PEEL]]:
...
; CHECK-NEXT:    br i1 [[EC1]], label %[[LOOP_HEADER]], label %[[LOOPEXIT_PEEL_BEGIN:.*]], !llvm.loop [[LOOP8:![0-9]+]]
