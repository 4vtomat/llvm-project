...
; RUN: -riscv-v-vector-bits-min=128 -sifive-vectorize-assume-optimizable-strided-accesses -sifive-unroll-assume-optimizable-strided-accesses -debug -S %s 2>&1 | FileCheck %s
; RUN: opt -passes="lto-pre-link<O2>" -mtriple=riscv64 -mattr=+v \
; RUN: -riscv-v-vector-bits-min=128 -sifive-vectorize-assume-optimizable-strided-accesses -sifive-unroll-assume-optimizable-strided-accesses -debug -S %s 2>&1 | FileCheck %s

; Skipping unroll in pre-link is controlled by option "sifive-unroll-assume-optimizable-strided-accesses".
; The option "sifive-vectorize-assume-optimizable-strided-accesses" also has to be specified because
; LoopUnrollPass happens after LoopVectorize in the middle-end pipeline. We won't go in the unroll check
; if the loop is already vectorized, so we have to skip it too.
; This file tests `LoopUnrollPass.cpp::HasOnlyNonUnitStrideMemoryAccesses`.
...
; CHECK: Bail out loop unroll in pre-link stage when there is only non-unit stride memory accesses.
entry:
