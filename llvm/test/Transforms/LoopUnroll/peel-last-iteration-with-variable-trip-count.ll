...
; CHECK-NEXT:    [[PEELCOUNTXSTEP_EXIT:%.*]] = sub i32 [[N]], 1
; CHECK-NEXT:    [[EC1:%.*]] = icmp ne i32 [[IV_NEXT1]], [[PEELCOUNTXSTEP_EXIT]]
...
; CHECK-NEXT:    [[IV_NEXT:%.*]] = add i32 [[TMP2]], 1
; CHECK-NEXT:    [[EC:%.*]] = icmp ne i32 [[IV_NEXT]], [[N]]
; CHECK-NEXT:    br i1 [[EC]], label %[[EXIT_PEEL_NEXT:.*]], label %[[EXIT_PEEL_NEXT]]
; CHECK:       [[EXIT_PEEL_NEXT]]:
; CHECK-NEXT:    br label %[[LOOP_PEEL_NEXT:.*]]
...
; CHECK-NEXT:    br label %[[LOOP:.*]]
; CHECK:       [[LOOP]]:
; CHECK-NEXT:    [[TMP3:%.*]] = phi i32 [ 0, %[[ENTRY_SPLIT]] ], [ [[IV_NEXT1:%.*]], %[[LOOP]] ]
; CHECK-NEXT:    [[C_PEEL:%.*]] = icmp ne i32 [[TMP3]], [[SUB]]
; CHECK-NEXT:    [[SEL_LCSSA:%.*]] = select i1 [[C_PEEL]], i32 1, i32 2
; CHECK-NEXT:    call void @foo(i32 [[SEL_LCSSA]])
; CHECK-NEXT:    [[IV_NEXT1]] = add nuw i32 [[TMP3]], 1
; CHECK-NEXT:    [[PEELCOUNTXSTEP_EXIT:%.*]] = sub i32 [[N]], 1
; CHECK-NEXT:    [[EC1:%.*]] = icmp ne i32 [[IV_NEXT1]], [[PEELCOUNTXSTEP_EXIT]]
; CHECK-NEXT:    br i1 [[EC1]], label %[[LOOP]], label %[[EXIT_PEEL_BEGIN_LOOPEXIT:.*]], !llvm.loop [[LOOP3:![0-9]+]]
; CHECK:       [[EXIT_PEEL_BEGIN_LOOPEXIT]]:
; CHECK-NEXT:    [[DOTPH:%.*]] = phi i32 [ [[IV_NEXT1]], %[[LOOP]] ]
; CHECK-NEXT:    br label %[[EXIT_PEEL_BEGIN]]
; CHECK:       [[EXIT_PEEL_BEGIN]]:
...
; CHECK-NEXT:    br label %[[LOOP_PEEL:.*]]
; CHECK:       [[LOOP_PEEL]]:
; CHECK-NEXT:    [[C_PEEL1:%.*]] = icmp ne i32 [[TMP2]], [[SUB]]
; CHECK-NEXT:    [[SEL_LCSSA1:%.*]] = select i1 [[C_PEEL1]], i32 1, i32 2
; CHECK-NEXT:    call void @foo(i32 [[SEL_LCSSA1]])
; CHECK-NEXT:    [[IV_NEXT:%.*]] = add i32 [[TMP2]], 1
; CHECK-NEXT:    [[EC:%.*]] = icmp ne i32 [[IV_NEXT]], [[N]]
; CHECK-NEXT:    br i1 [[EC]], label %[[EXIT_PEEL_NEXT:.*]], label %[[EXIT_PEEL_NEXT]]
; CHECK:       [[EXIT_PEEL_NEXT]]:
...
; CHECK-NEXT:    [[PEELCOUNTXSTEP_EXIT:%.*]] = sub i32 [[N]], 1
; CHECK-NEXT:    [[EC:%.*]] = icmp ne i32 [[IV_NEXT]], [[PEELCOUNTXSTEP_EXIT]]
; CHECK-NEXT:    br i1 [[EC]], label %[[LOOP]], label %[[EXIT_PEEL_BEGIN_LOOPEXIT:.*]], !llvm.loop [[LOOP4:![0-9]+]]
; CHECK:       [[EXIT_PEEL_BEGIN_LOOPEXIT]]:
...
; CHECK-NEXT:    [[PEELCOUNTXSTEP_EXIT:%.*]] = sub i32 [[N]], 1
; CHECK-NEXT:    [[EC1:%.*]] = icmp ne i32 [[IV_NEXT1]], [[PEELCOUNTXSTEP_EXIT]]
; CHECK-NEXT:    br i1 [[EC1]], label %[[LOOP]], label %[[EXIT_PEEL_BEGIN_LOOPEXIT:.*]], !llvm.loop [[LOOP5:![0-9]+]]
; CHECK:       [[EXIT_PEEL_BEGIN_LOOPEXIT]]:
; CHECK-NEXT:    [[DOTPH:%.*]] = phi i32 [ [[IV_NEXT1]], %[[LOOP]] ]
; CHECK-NEXT:    br label %[[EXIT_PEEL_BEGIN]]
; CHECK:       [[EXIT_PEEL_BEGIN]]:
...
; CHECK-NEXT:    br i1 [[EC]], label %[[OUTER_HEADER_LOOPEXIT_PEEL_NEXT:.*]], label %[[OUTER_HEADER_LOOPEXIT_PEEL_NEXT]]
...
; CHECK-NEXT:    [[IV_NEXT1]] = add nuw i32 [[IV1]], 1
; CHECK-NEXT:    [[PEELCOUNTXSTEP_EXIT:%.*]] = sub i32 [[N]], 1
; CHECK-NEXT:    [[EC1:%.*]] = icmp eq i32 [[IV_NEXT1]], [[PEELCOUNTXSTEP_EXIT]]
; CHECK-NEXT:    br i1 [[EC1]], label %[[OUTER_HEADER_LOOPEXIT_PEEL_BEGIN_LOOPEXIT]], label %[[INNER_HEADER]], !llvm.loop [[LOOP6:![0-9]+]]
;
entry:
  %sub = add i32 %n, -1
