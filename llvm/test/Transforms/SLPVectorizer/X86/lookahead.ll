...
; SSE-NEXT:    [[IDXA2:%.*]] = getelementptr inbounds double, ptr [[A]], i64 2
; SSE-NEXT:    [[C0:%.*]] = load double, ptr [[C:%.*]], align 8
; SSE-NEXT:    [[D0:%.*]] = load double, ptr [[D:%.*]], align 8
; SSE-NEXT:    [[A2:%.*]] = load double, ptr [[IDXA2]], align 8
; SSE-NEXT:    [[A1:%.*]] = load double, ptr [[IDXA1]], align 8
; SSE-NEXT:    [[TMP0:%.*]] = load <2 x double>, ptr [[A]], align 8
; SSE-NEXT:    [[TMP1:%.*]] = load <3 x double>, ptr [[B:%.*]], align 8
; SSE-NEXT:    [[TMP2:%.*]] = shufflevector <3 x double> [[TMP1]], <3 x double> poison, <2 x i32> <i32 0, i32 2>
; SSE-NEXT:    [[TMP3:%.*]] = fsub fast <2 x double> [[TMP0]], [[TMP2]]
; SSE-NEXT:    [[TMP4:%.*]] = insertelement <2 x double> poison, double [[C0]], i32 0
; SSE-NEXT:    [[TMP5:%.*]] = insertelement <2 x double> [[TMP4]], double [[A2]], i32 1
; SSE-NEXT:    [[TMP6:%.*]] = shufflevector <3 x double> [[TMP1]], <3 x double> poison, <2 x i32> <i32 poison, i32 1>
; SSE-NEXT:    [[TMP7:%.*]] = insertelement <2 x double> [[TMP6]], double [[D0]], i32 0
; SSE-NEXT:    [[TMP8:%.*]] = fsub fast <2 x double> [[TMP5]], [[TMP7]]
; SSE-NEXT:    [[TMP9:%.*]] = fadd fast <2 x double> [[TMP3]], [[TMP8]]
; SSE-NEXT:    store <2 x double> [[TMP9]], ptr [[S:%.*]], align 8
; SSE-NEXT:    store double [[A1]], ptr [[EXT1:%.*]], align 8
; SSE-NEXT:    ret void
;
...
; SSE-NEXT:    store <2 x double> [[TMP9]], ptr [[S:%.*]], align 8
; SSE-NEXT:    store double [[A1]], ptr [[EXT1:%.*]], align 8
; SSE-NEXT:    store double [[A1]], ptr [[EXT2:%.*]], align 8
; SSE-NEXT:    store double [[A1]], ptr [[EXT3:%.*]], align 8
