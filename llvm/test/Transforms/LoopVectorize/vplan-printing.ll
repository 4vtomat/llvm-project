...
; RUN: opt -passes=loop-vectorize -debug-only=loop-vectorize -force-vector-interleave=1 -force-vector-width=4 -prefer-inloop-reductions -hints-allow-reordering=true -enable-interleaved-mem-accesses=true -enable-masked-interleaved-mem-accesses -force-widen-divrem-via-safe-divisor=0 -disable-output %s 2>&1 | FileCheck %s
...
; CHECK-NEXT:   EMIT vp<%8> = not ir<%ifcond>
...
<<<<<<< HEAD
; CHECK-NEXT:    BLEND ir<%st.value> = ir<%ld.value>/vp<%8> ir<%fadd>/ir<%ifcond>
; CHECK-NEXT:    CLONE ir<%st.addr> = getelementptr inbounds ir<%dest>, vp<%3>
; CHECK-NEXT:    vp<%16> = vector-pointer ir<%st.addr>
; CHECK-NEXT:    WIDEN store vp<%16>, ir<%st.value>
=======
