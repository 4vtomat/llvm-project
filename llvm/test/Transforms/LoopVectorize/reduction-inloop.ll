...
; RUN: opt < %s -passes=loop-vectorize,dce,instcombine -force-vector-interleave=1 -force-vector-width=4 -prefer-inloop-reductions -hints-allow-reordering=true -S | FileCheck %s
...
; CHECK-NEXT:    [[COMBINED_REDUCE:%.*]] = add <4 x i32> [[VEC_IND]], [[WIDE_LOAD]]
; CHECK-NEXT:    [[TMP2:%.*]] = call i32 @llvm.vector.reduce.add.v4i32(<4 x i32> [[COMBINED_REDUCE]])
; CHECK-NEXT:    [[TMP5:%.*]] = add i32 [[TMP2]], [[VEC_PHI]]
...
; CHECK-NEXT:    [[TMP8:%.*]] = fcmp ogt <4 x float> [[WIDE_LOAD]], <float 2.000000e+00, float 2.000000e+00, float 2.000000e+00, float 2.000000e+00>
; CHECK-NEXT:    [[TMP7:%.*]] = and <4 x i1> [[TMP6]], [[TMP8]]
; CHECK-NEXT:    [[TMP14:%.*]] = xor <4 x i1> [[TMP8]], <i1 true, i1 true, i1 true, i1 true>
; CHECK-NEXT:    [[TMP9:%.*]] = and <4 x i1> [[TMP6]], [[TMP14]]
...
; CHECK-NEXT:    [[TMP47:%.*]] = select <4 x i1> [[NOT_]], <4 x i1> <i1 true, i1 true, i1 true, i1 true>, <4 x i1> [[TMP44]]
