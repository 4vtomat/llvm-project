; NOTE: Assertions have been autogenerated by utils/update_test_checks.py UTC_ARGS: --tool cat
; REQUIRES: asserts
; RUN: opt -S -passes=loop-vectorize -debug-only=loop-vectorize < %s 2>%t | FileCheck %s
; RUN: cat /scratch/michaelb/build_staging/test/Transforms/LoopVectorize/AArch64/Output/sve-tail-folding-forced.ll.tmp | FileCheck %s --check-prefix=VPLANS
...
; VPLANS-NEXT:     EMIT vp<%4> = TC > VF ? TC - VF : 0 vp<%2>
; VPLANS-NEXT:     EMIT vp<%5> = active lane mask vp<%3> vp<%2>
