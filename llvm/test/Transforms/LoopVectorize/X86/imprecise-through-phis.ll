...
; SSE:       do.add:
; SSE-NEXT:    [[TOT_NEW:%.*]] = fadd fast double [[BC_MERGE_RDX]], [[NEXTVAL]]
; SSE-NEXT:    br label [[MIDDLE_BLOCK]]
; SSE:       no.add:
; SSE-NEXT:    br label [[MIDDLE_BLOCK]]
; SSE:       next.iter:
; SSE-NEXT:    [[TMP11]] = phi double [ [[BC_MERGE_RDX]], [[NO_ADD]] ], [ [[TOT_NEW]], [[DO_ADD]] ]
; SSE-NEXT:    [[I_NEXT]] = add i32 [[I]], 1
; SSE-NEXT:    [[AGAIN:%.*]] = icmp ult i32 [[I_NEXT]], 32
; SSE-NEXT:    br i1 [[AGAIN]], label [[VECTOR_BODY]], label [[DONE:%.*]]
; SSE:       done:
; SSE-NEXT:    [[TOT_NEXT_LCSSA:%.*]] = phi double [ [[TMP11]], [[MIDDLE_BLOCK]] ]
...
; AVX-NEXT:    [[TMP5:%.*]] = getelementptr double, ptr [[TMP4]], i32 12
; AVX-NEXT:    [[WIDE_LOAD6:%.*]] = load <4 x double>, ptr [[TMP8]], align 8
; AVX-NEXT:    [[WIDE_LOAD4:%.*]] = load <4 x double>, ptr [[TMP3]], align 8
; AVX-NEXT:    [[WIDE_LOAD5:%.*]] = load <4 x double>, ptr [[TMP6]], align 8
; AVX-NEXT:    [[WIDE_LOAD7:%.*]] = load <4 x double>, ptr [[TMP5]], align 8
; AVX-NEXT:    [[TMP15:%.*]] = fcmp fast une <4 x double> [[WIDE_LOAD6]], splat (double 4.200000e+01)
; AVX-NEXT:    [[TMP7:%.*]] = fcmp fast une <4 x double> [[WIDE_LOAD4]], splat (double 4.200000e+01)
; AVX-NEXT:    [[TMP10:%.*]] = fcmp fast une <4 x double> [[WIDE_LOAD5]], splat (double 4.200000e+01)
; AVX-NEXT:    [[TMP9:%.*]] = fcmp fast une <4 x double> [[WIDE_LOAD7]], splat (double 4.200000e+01)
; AVX-NEXT:    [[TMP19:%.*]] = fadd fast <4 x double> [[VEC_PHI3]], [[WIDE_LOAD6]]
; AVX-NEXT:    [[TMP11:%.*]] = fadd fast <4 x double> [[VEC_PHI1]], [[WIDE_LOAD4]]
; AVX-NEXT:    [[TMP12:%.*]] = fadd fast <4 x double> [[VEC_PHI2]], [[WIDE_LOAD5]]
; AVX-NEXT:    [[TMP13:%.*]] = fadd fast <4 x double> [[VEC_PHI4]], [[WIDE_LOAD7]]
; AVX-NEXT:    [[PREDPHI9]] = select <4 x i1> [[TMP15]], <4 x double> [[TMP19]], <4 x double> [[VEC_PHI3]]
; AVX-NEXT:    [[PREDPHI7]] = select <4 x i1> [[TMP7]], <4 x double> [[TMP11]], <4 x double> [[VEC_PHI1]]
; AVX-NEXT:    [[PREDPHI8]] = select <4 x i1> [[TMP10]], <4 x double> [[TMP12]], <4 x double> [[VEC_PHI2]]
; AVX-NEXT:    [[PREDPHI10]] = select <4 x i1> [[TMP9]], <4 x double> [[TMP13]], <4 x double> [[VEC_PHI4]]
...
; AVX:       middle.block:
; AVX-NEXT:    [[BIN_RDX:%.*]] = fadd fast <4 x double> [[PREDPHI7]], [[PREDPHI9]]
; AVX-NEXT:    [[BIN_RDX10:%.*]] = fadd fast <4 x double> [[PREDPHI8]], [[BIN_RDX]]
; AVX-NEXT:    [[BIN_RDX11:%.*]] = fadd fast <4 x double> [[PREDPHI10]], [[BIN_RDX10]]
