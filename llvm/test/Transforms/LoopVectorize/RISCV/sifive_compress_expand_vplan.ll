...
; CHECK-NEXT:     EMIT vp<%2> = CANONICAL-INDUCTION ir<0>, vp<%index.evl.next>
; CHECK-NEXT:     EXPLICIT-VECTOR-LENGTH-BASED-IV-PHI vp<%3> = phi ir<0>, vp<%index.evl.next>
; CHECK-NEXT:     EMIT ir<%ret.011> = monotonic-phi ir<0>, ir<%inc>
; CHECK-NEXT:     EMIT vp<%avl> = sub vp<%0>, vp<%3>
; CHECK-NEXT:     EMIT vp<%4> = EXPLICIT-VECTOR-LENGTH vp<%avl>
; CHECK-NEXT:     CLONE ir<%arrayidx> = getelementptr inbounds ir<%b>, vp<{{.*}}>
; CHECK-NEXT:     vp<{{.*}}> = vector-pointer ir<%arrayidx>
; CHECK-NEXT:     WIDEN ir<{{.*}}> = vp.load vp<{{.*}}>, vp<{{.*}}>	unit-strided
; CHECK-NEXT:     WIDEN ir<%tobool.not> = icmp eq ir<%0>, ir<0>
; CHECK-NEXT:     EMIT vp<{{.*}}> = not ir<%tobool.not>
; CHECK-NEXT:     CLONE ir<%idx.ext> = sext ir<%ret.011>
; CHECK-NEXT:     CLONE ir<%add.ptr> = getelementptr ir<%a>, ir<%idx.ext>
; CHECK-NEXT:     vp<{{.*}}> = vector-pointer ir<{{.*}}>
; CHECK-NEXT:     WIDEN vp.store vp<{{.*}}>, ir<{{.*}}>, vp<{{.*}}>, vp<{{.*}}>        unit-strided
...
; CHECK-NEXT:     vp<{{.*}}> = vector-pointer ir<%arrayidx2>
; CHECK-NEXT:     WIDEN ir<{{.*}}> = vp.load vp<{{.*}}>, vp<{{.*}}>, vp<{{.*}}>        unit-strided
; CHECK-NEXT:     CLONE ir<%arrayidx4> = getelementptr ir<%a>, vp<{{.*}}>
; CHECK-NEXT:     vp<{{.*}}> = vector-pointer ir<%arrayidx4>
; CHECK-NEXT:     WIDEN vp.store vp<{{.*}}>, ir<{{.*}}>, vp<{{.*}}>, vp<{{.*}}>        unit-strided
