...
; CHECK-NEXT:    [[TMP6:%.*]] = icmp ule i64 [[TMP5]], 1
; CHECK-NEXT:    br i1 [[TMP6]], label %[[SCALAR_PH:.*]], label %[[VECTOR_SCEVCHECK:.*]]
...
; CHECK:       [[VECTOR_PH]]:
; CHECK-NEXT:    [[N_VEC:%.*]] = sub i64 [[TMP5]], 1
...
; CHECK-NEXT:    [[TMP45:%.*]] = sub i64 [[N_VEC]], [[EVL_BASED_IV]]
; CHECK-NEXT:    [[TMP46:%.*]] = call i32 @llvm.experimental.get.vector.length.i64(i64 [[TMP45]], i32 4, i1 true)
; CHECK-NEXT:    [[TMP47:%.*]] = mul i64 [[EVL_BASED_IV]], 3
; CHECK-NEXT:    [[OFFSET_IDX:%.*]] = add i64 [[X_I64]], [[TMP47]]
; CHECK-NEXT:    [[TMP48:%.*]] = add i64 [[OFFSET_IDX]], 0
; CHECK-NEXT:    [[TMP51:%.*]] = getelementptr i16, ptr [[A]], i64 [[TMP48]]
; CHECK-NEXT:    call void @llvm.experimental.vp.strided.store.nxv4i16.p0.i64(<vscale x 4 x i16> zeroinitializer, ptr align 2 [[TMP51]], i64 6, <vscale x 4 x i1> shufflevector (<vscale x 4 x i1> insertelement (<vscale x 4 x i1> poison, i1 true, i64 0), <vscale x 4 x i1> poison, <vscale x 4 x i32> zeroinitializer), i32 [[TMP46]]), !alias.scope [[META0:![0-9]+]], !noalias [[META3:![0-9]+]]
; CHECK-NEXT:    [[TMP52:%.*]] = zext i32 [[TMP46]] to i64
; CHECK-NEXT:    [[INDEX_NEXT]] = add i64 [[TMP52]], [[EVL_BASED_IV]]
