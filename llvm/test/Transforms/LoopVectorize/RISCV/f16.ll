...
; RUN: opt < %s -passes=loop-vectorize -mtriple riscv64 -mattr=+v -S --riscv-use-vla-vectorizer=false | FileCheck %s -check-prefix=NO-ZVFHMIN
; RUN: opt < %s -passes=loop-vectorize -mtriple riscv64 -mattr=+v,+zvfhmin -S --riscv-use-vla-vectorizer=false | FileCheck %s -check-prefix=ZVFHMIN
