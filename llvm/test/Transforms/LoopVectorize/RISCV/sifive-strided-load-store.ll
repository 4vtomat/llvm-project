...
; RUN: opt < %s -loop-vectorize -instcombine -force-vector-interleave=1 -mtriple=riscv32 -mattr=+v -riscv-v-vector-bits-min=256 -S | FileCheck %s --check-prefixes=RV32V
; RUN: opt < %s -loop-vectorize -instcombine -force-vector-interleave=1 -mtriple=riscv64 -mattr=+v -riscv-v-vector-bits-min=256 -S | FileCheck %s --check-prefixes=RV64V
; RUN: opt < %s -loop-vectorize -instcombine -force-vector-interleave=1 -mtriple=riscv32 -mattr=+f,zve32f -riscv-v-vector-bits-min=256 -S | FileCheck %s --check-prefixes=RV32ZVE32F
; RUN: opt < %s -loop-vectorize -instcombine -force-vector-interleave=1 -mtriple=riscv64 -mattr=+f,zve32f -riscv-v-vector-bits-min=256 -S | FileCheck %s --check-prefixes=RV64ZVE32f
