...
; RUN: opt -S < %s -mtriple=riscv64 -mattr=+d,+v -riscv-v-vector-bits-min=512 -vector-lmul-max=1 -scalable-vectorization=only -riscv-prefer-predicated-vector-ops -prefer-predicate-over-epilogue=predicate-dont-vectorize -gvn -loop-vectorize | FileCheck %s
; RUN: opt -S < %s -mtriple=riscv64 -mattr=+d,+v -riscv-v-vector-bits-min=512 -vector-lmul-max=1 -scalable-vectorization=only -riscv-prefer-predicated-vector-ops -riscv-prefer-post-fix-start-value=false -prefer-predicate-over-epilogue=predicate-dont-vectorize -gvn -loop-vectorize | FileCheck %s --check-prefix=CHECK-NO-POSTSV
