...
; RUN: opt < %s -passes=loop-vectorize -mtriple riscv64 -mattr=+v -S --riscv-use-vla-vectorizer=false | FileCheck %s -check-prefix=NO-ZVFBFMIN
; RUN: opt < %s -passes=loop-vectorize -mtriple riscv64 -mattr=+v,+zvfbfmin -S --riscv-use-vla-vectorizer=false | FileCheck %s -check-prefix=ZVFBFMIN
