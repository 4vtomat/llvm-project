; REQUIRES: asserts
 ...
 ; IF-EVL-NEXT:     EMIT vp<[[IV:%.+]]> = CANONICAL-INDUCTION ir<0>, vp<[[IV_EVL_NEXT:%.+]]>
 ; IF-EVL-NEXT:     EXPLICIT-VECTOR-LENGTH-BASED-IV-PHI vp<[[EVL_PHI:%.+]]> = phi ir<0>, vp<[[IV_EVL_NEXT:%.+]]>
 ...
 ; IF-EVL-NEXT:     WIDEN-SELECT ir<[[SELECT:%.+]]> = select ir<[[CMP]]>, ir<[[LD2]]>, ir<[[SUB]]>
 ; IF-EVL-NEXT:     WIDEN ir<[[ADD:%.+]]> = add ir<[[SELECT]]>, ir<[[LD1]]>
 ...
 ; IF-EVL-NEXT:     EMIT vp<[[IV_EVL_NEXT]]> = add nuw vp<[[CAST]]>, vp<[[EVL_PHI]]>
 ; IF-EVL-NEXT:     EMIT branch-on-count vp<[[IV_EVL_NEXT]]>, vp<[[VTC]]>
